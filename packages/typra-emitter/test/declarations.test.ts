...
function makeType(
  ...
  node.methods = (opts?.methods ?? []).map((m) => ({
    ...m,
    params: m.params ?? {},
...
describe("closed polymorphic dispatch", () => {
  it("uses the lowered discriminator contract instead of abstractness", () => {
    ...
    assert.equal(
      isClosedPolymorphicDispatch({ ...base, isClosed: true }),
      true,
    );
    assert.equal(
      isClosedPolymorphicDispatch({ ...base, isClosed: false }),
      false,
    );
  });
  ...
  describe("property lowering diagnostics", () => {
    it("rejects complex defaults so they cannot disable required-field guards", () => {
      ...
      const lowered = resolveProperty(
        program,
        property,
        new Set(["FixtureOwner"]),
        "Test",
        "",
      );
      ...
      assert.equal(
        diagnostics[0].code,
        "typra-emitter-unsupported-complex-default",
      );
      assert.equal(diagnostics[0].severity, "error");
      assert.match(
        diagnostics[0].message,
        /does not support complex object defaults/,
      );
  ...
  it("lowers closed enums separately from open abstract discriminators", () => {
    const closedText = makeType(
      "ClosedText",
      [makeProp("kind", "string", { isScalar: true, defaultValue: "text" })],
      { base: { namespace: "Test", name: "ClosedContent" } },
    );
    const closedContent = makeType(
      "ClosedContent",
    ...
    const closedDispatch = lowerFile(
      closedContent,
      buildTestRegistry(),
      new Set(["ClosedContent"]),
    ).types[0].polymorphicDispatch!;
    const openDispatch = lowerFile(
      connectionType,
      buildTestRegistry(),
      new Set(["Connection"]),
    ).types[0].polymorphicDispatch!;
    ...
    const closedRegistry = TypeRegistry.fromTypeGraph([
      closedContent,
      closedText,
    ]);
    const closedFile = lowerFile(
      closedContent,
      closedRegistry,
      new Set(["ClosedContent"]),
    );
    ...
    assert.match(
      goSource,
      /unknown ClosedContent discriminator field 'kind' value/,
    );
  });
  ...
  it("lets a schema-declared wildcard subtype own the open decision", () => {
    ...
    assert.deepEqual(
      dispatch.variants.map((v) => v.value),
      ["known"],
    );
  });
  ...
  it("lets a non-abstract base absorb permitted discriminator values no subtype claims", () => {
    ...
    const baseProperty = makeType(
      "BaseProperty",
      [
        makeProp("kind", "SimpleTypes", {
          allowedValues: ["string", "boolean", "array", "object"],
        }),
      ],
    ...
    assert.ok(
      ...
      "non-abstract base must keep a self-reference fallback",
    );
...
const toolResult = makeType(
  ...
  [makeProp("parts", "ContentPart", { isCollection: true, type: contentPart })],
  {
    factories: [
      {
        name: "text",
        sets: { parts: [{ kind: "text", value: "{value}" }] },
        params: { value: "string" },
      },
    ],
  },
...
const apiKeyConnection = makeType(
  "ApiKeyConnection",
  [
    ...
    methods: [
      {
        name: "text",
        returns: "string",
        description: "Get the text value",
        optional: false,
        sync: false,
      },
    ],
...
describe("classifyProperty", () => {
  ...
  it("classifies optional scalar property", () => {
    const prop = makeProp("reason", "string", {
      isScalar: true,
      isOptional: true,
    });
    const cat = classifyProperty(prop, polyNames);
  ...
  it("classifies collection of scalars", () => {
    const prop = makeProp("tags", "string", {
      isScalar: true,
      isCollection: true,
    });
    const cat = classifyProperty(prop, polyNames);
  ...
  it("classifies collection of complex types", () => {
    const prop = makeProp("parts", "ContentPart", {
      isCollection: true,
      type: contentPart,
    });
    const cat = classifyProperty(prop, polyNames);
  ...
  it("dict takes priority over collection", () => {
    ...
    const prop = makeProp("extra", "string", {
      isDict: true,
      isCollection: true,
    });
    const cat = classifyProperty(prop, polyNames);
...
describe("lowerType", () => {
  ...
  it("lowers a simple type with scalar fields", () => {
    ...
    assert.deepEqual(decl.fields[0].category, {
      kind: "scalar",
      scalarType: "string",
    });
  ...
  it("lowers a type with complex collection", () => {
    ...
    assert.deepEqual(decl.fields[1].category, {
      kind: "collection_complex",
      typeName: "ContentPart",
    });
    // metadata is dict
  ...
  it("lowers polymorphic dispatch", () => {
    ...
    assert.equal(
      decl.polymorphicDispatch!.variants[0].typeName.name,
      "TextPart",
    );
  ...
  it("lowers non-abstract polymorphic base with default", () => {
    ...
    assert.equal(
      decl.polymorphicDispatch!.defaultVariant!.isSelfReference,
      true,
    );
  });
  ...
  it("keeps explicit keyed-collection metadata for a 2nd same-element collection whose prop.type is unset", () => {
    ...
    const inputs = makeProp("inputs", "Binding", {
      isCollection: true,
      isNamedCollection: true,
      type: namedBinding,
    });
    const outputs = makeProp("outputs", "Binding", {
      isCollection: true,
      isNamedCollection: true,
    }); // type UNSET (cycle quirk)
    ...
    const out = decl.collectionHelpers.find(
      (h) => h.propertyName === "outputs",
    )!;
    assert.equal(
      out.hasNameProperty,
      true,
      "2nd same-element collection must retain the explicit keyed shape",
    );
    assert.deepEqual(out.innerFields, ["value"]);
  });

  it("does not infer keyed serialization from an ordinary element name field", () => {
    const requests = makeProp("pendingToolRequests", "Binding", {
      isCollection: true,
      type: namedBinding,
    });
    const checkpoint = makeType("Checkpoint", [requests]);
    const checkpointRegistry = TypeRegistry.fromTypeGraph([
      checkpoint,
      namedBinding,
    ]);
    const decl = lowerType(checkpoint, checkpointRegistry, new Set());
    const helper = decl.collectionHelpers.find(
      (h) => h.propertyName === "pendingToolRequests",
    )!;
    ...
    assert.match(
      source,
      /The schema declares an ordered collection[\s\S]*return \[item\.save\(context\) for item in items\]/,
    );
  ...
  it("recovers keyed-collection detection via structural isNamedCollection when the element type lacks a real name field", () => {
    ...
    const rawBinding = makeType("RawBinding", [
      makeProp("value", "string", { isScalar: true }),
    ]);
    const outputs = makeProp("outputs", "RawBinding", {
      isCollection: true,
      isNamedCollection: true,
    }); // type UNSET, raw element has NO name
    ...
    assert.equal(
      out.hasNameProperty,
      true,
      "structural isNamedCollection must recover keyed detection even when the raw element type has no name field",
    );
  });
...
describe("lowerFile", () => {
  ...
  describe("Swift emitter inheritance", () => {
    it("does not emit invalid struct conformance for non-polymorphic model inheritance", () => {
      ...
      const content = emitSwiftFile(
        {
          ...
          types: [child].map((node) =>
            lowerType(
              node,
              TypeRegistry.fromTypeGraph([base, child]),
              new Set(),
            ),
          ),
          imports: [],
        ...
        new SwiftExprVisitor(TypeRegistry.fromTypeGraph([base, child])),
        new Set(),
      );
      ...
      assert.doesNotMatch(
        content,
        /public struct ChildModel: TypraModel, BaseModel/,
      );
    });
  });
  ...
  describe("Go emitter dispatch hardening", () => {
    it("keeps abstract scalar coercions reachable before discriminator validation", () => {
      const tokenConnection = makeType(
        "TokenConnection",
        [
      ...
      const connectionWithCoercion = makeType(
        "ConnectionWithCoercion",
        [
          makeProp("kind", "string", { isScalar: true }),
          makeProp("endpoint", "string", { isScalar: true }),
        ],
        {
          ...
          coercions: [
            {
              scalar: "string",
              expansion: { kind: "token", endpoint: "{value}" },
            },
          ],
        },
      );
      ...
      const coercionIndex = code.indexOf(
        "// Handle alternate scalar representations",
      );
      const dispatchIndex = code.indexOf(
        "// Handle polymorphic types based on discriminator",
      );
      assert.ok(coercionIndex >= 0, "expected generated coercion block");
      assert.ok(
        dispatchIndex >= 0,
        "expected generated polymorphic dispatch block",
      );
      assert.ok(
        coercionIndex < dispatchIndex,
        "scalar coercions must run before abstract dispatch errors",
      );
      ...
      assert.doesNotMatch(
        loadBody,
        /unknown ConnectionWithCoercion discriminator/,
      );
      ...
      assert.match(
        loadBody,
        /result\.raw\[key\] = cloneConnectionWithCoercionRawValue\(value\)/,
      );
    });
    ...
    it("keeps an abstract CLOSED dispatch terminal so no unreachable load tail is emitted", () => {
      ...
      assert.match(
        loadBody,
        /return nil, fmt\.Errorf\("unknown ClosedPartBase discriminator field 'kind' value: %s", discriminator\)/,
      );
      assert.match(
        loadBody,
        /return nil, fmt\.Errorf\("missing ClosedPartBase discriminator property: kind"\)/,
      );
    ...
    it("deterministically disambiguates normalized leading-underscore collisions", () => {
      ...
      assert.match(
        code,
        /FieldTime string `json:"fieldTime" yaml:"fieldTime"`/,
      );
    ...
    it("preserves unmodeled payload only for self-referential open defaults", () => {
      ...
      assert.match(
        code,
        /type OpenConnection struct \{[\s\S]*\traw map\[string\]interface\{\}/,
      );
      assert.match(
        code,
      ...
      assert.match(
        code,
        /func cloneOpenConnectionRawValue\(value interface\{\}\) interface\{\}/,
      );
      assert.match(
        code,
        /result\.raw\[key\] = cloneOpenConnectionRawValue\(value\)/,
      );
      assert.match(
        code,
        /for key, value := range obj\.raw \{[\s\S]*result\[key\] = cloneOpenConnectionRawValue\(value\)/,
      );
      assert.ok(
        code.indexOf("for key, value := range obj.raw") <
          code.indexOf('result["kind"] = obj.Kind'),
        "modeled fields must overwrite any retained raw payload",
      );
      assert.doesNotMatch(
        code,
        /type ApiKeyConnection struct \{[\s\S]*\traw map\[string\]interface\{\}/,
      );
    });
    ...
    it("flattens inherited base fields into child structs (extends)", () => {
      ...
      const conn = makeType(
        ...
        [
          makeProp("kind", "string", { isScalar: true }),
          makeProp("authenticationMode", "string", {
            isScalar: true,
            isOptional: true,
          }),
          makeProp("usageDescription", "string", {
            isScalar: true,
            isOptional: true,
          }),
        ],
      ...
      const structBody = code.slice(
        structStart,
        code.indexOf("}", structStart),
      );
    ...
    it("flattens named collection helpers from ancestors emitted in another file", () => {
      ...
      assert.match(
        code,
        /type FunctionTool struct \{[\s\S]*Bindings \[\]Binding/,
      );
      assert.match(
        code,
  ...
  describe("Go test emitter optional assertions", () => {
        ...
        examples: [
          {
            ...
            json: [
              '{"__time":"internal","time":"public","fieldTime":"existing"}',
            ],
    ...
    it("nil-checks and dereferences optional string properties", () => {
      const instructions = makeProp("instructions", "string", {
        isScalar: true,
        isOptional: true,
      });
    ...
    it("keeps whitespace-sensitive multiline YAML trim-proof", () => {
      ...
      instructions.samples = [
        { sample: { instructions: expected }, description: "" },
      ];
      const prompty = makeType("Prompty", [instructions]);
      ...
      instructions.samples = [
        { sample: { instructions: blockValue }, description: "" },
      ];
      const blockContext = buildBaseTestContext(
        prompty,
        "prompty",
        goTestOptions,
      );
      ...
      instructions.samples = [
        { sample: { instructions: trailingValue }, description: "" },
      ];
      const trailingContext = buildBaseTestContext(
        prompty,
        "prompty",
        goTestOptions,
      );
      ...
      assert.deepEqual(trailingSpaceContext.examples[0].yaml, [
        'value: "first line with trailing space \\nsecond line\\n"',
        "",
      ]);

      const mixedWhitespaceValue =
      ...
      assert.deepEqual(mixedWhitespaceContext.examples[0].yaml, [
        'value: "first line with two spaces  \\n\\n  \\nlast line with three spaces   \\n"',
        "",
      ]);
      assert.equal(
        mixedWhitespaceContext.examples[0].sample.value,
        mixedWhitespaceValue,
      );
      ...
      assert.deepEqual(leadingTabContext.examples[0].yaml, [
      ...
      assert.deepEqual(whitespaceContext.examples[0].yaml, [
        'value: "\\n"',
        "",
      ]);
      ...
      assert.doesNotMatch(
        unicodeContext.examples[0].yaml.join("\n"),
        /\|[-+]?/,
      );
    });
  });
...
describe("open self-reference payload preservation", () => {
  ...
  it("emits exact, deep-cloned C# passthrough state", () => {
    ...
    assert.match(
      code,
      /discriminatorValue is not string discriminator \|\| discriminator == ""/,
    );
  ...
  it("emits exact, deep-cloned Python passthrough state", () => {
    ...
    assert.match(
      code,
      /if type\(instance\) is ContentPart:\s+instance\._raw = copy\.deepcopy\(data\)/,
    );
    assert.match(
      code,
...
describe("Rust emitter serde derives", () => {
  ...
  it("preserves unknown abstract discriminator payloads losslessly", () => {
    ...
    assert.match(
      code,
      /_ => ConnectionKind::Unknown \{\s+kind_name: kind_str\.to_string\(\),\s+raw: \{/,
    );
    ...
    assert.match(
      code,
      /ConnectionKind::Unknown \{ raw, \.\. \} => \{\s+for \(key, value\) in raw/,
    );
  });
  ...
  it("preserves open self-reference discriminator payloads losslessly", () => {
    ...
    assert.match(
      code,
      /_ => ContentPartKind::Custom \{\s+kind_name: kind_str\.to_string\(\),\s+raw: \{/,
    );
    assert.match(code, /raw\.remove\("kind"\);/);
    assert.match(
      code,
      /ContentPartKind::Custom \{ raw, \.\. \} => \{\s+for \(key, value\) in raw \{\s+if matches!\(key\.as_str\(\), "kind"\) \{ continue; \}/,
    );
  });

  it("initializes raw payloads for coerced self-reference variants", () => {
    const known = makeType(
      "CoercedOpenKnown",
      [makeProp("kind", "string", { isScalar: true, defaultValue: "known" })],
    ...
    const code = emitRustFile(
      file,
      new RustExprVisitor(coercionRegistry),
      new Set(["CoercedOpen"]),
    );
  ...
  it("uses concrete vectors for explicit empty defaults and options for absent defaults", () => {
    ...
    const collectionRegistry = TypeRegistry.fromTypeGraph([
      collectionModel,
      owner,
    ]);
    ...
    const code = emitRustFile(
      file,
      new RustExprVisitor(collectionRegistry),
      new Set(),
    );
  ...
  it("materializes explicit optional collection defaults as concrete vectors", () => {
    ...
    assert.match(
      code,
      /result\.insert\("tags"\.to_string\(\), serde_json::to_value\(&self\.tags\)/,
    );
    assert.match(
      code,
  ...
  it("materializes explicit collection defaults inside polymorphic variants", () => {
    ...
    assert.match(
      code,
      /result\.insert\("allowedTools"\.to_string\(\), serde_json::to_value\(allowed_tools\)/,
    );
...
describe("Rust test generator serde_roundtrip gating", () => {
  ...
  it("never emits integer-index nested-discriminator navigation", async () => {
    ...
    const code = emitRustTest({
      ...
      isAbstract: false,
      examples: [
        makeExample({ parts: [{ kind: "text", value: "hi" }] }, [
          "{",
  ...
  it("suppresses byte-identity when a REQUIRED field is authored at its omittable zero/empty value", async () => {
    ...
    const code = emitRustTest({
      ...
      isAbstract: false,
      examples: [
        makeExample({ title: "hi", count: 0, tags: [] }, [
          "{",
          '  "title": "hi",',
          '  "count": 0,',
          '  "tags": []',
