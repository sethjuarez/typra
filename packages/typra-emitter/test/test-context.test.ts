...
function sampleFor(
  ...
  const context = buildBaseTestContext(node, undefined, goTestOptions, (name) =>
    byName.get(name),
  );
  assert.equal(
    context.examples.length,
    1,
    "expected exactly one generated example",
  );
...
function exampleCountFor(node: TypeNode, types: TypeNode[] = []): number {
  const byName = new Map(types.map((type) => [type.typeName.name, type]));
  return buildBaseTestContext(node, undefined, goTestOptions, (name) =>
    byName.get(name),
  ).examples.length;
}
...
describe("test context — required complex sample synthesis", () => {
  ...
  it("leaves an optional complex property absent", () => {
    ...
    assert.ok(
      !("detail" in sampleFor(node, [detail])),
      "optional complex field must stay absent",
    );
  });
  ...
  it("wraps the synthesized payload for a required complex collection", () => {
    ...
    assert.deepEqual(sampleFor(node, [detail]).details, [
      { code: "detail-code" },
    ]);
  });
  ...
  it("recurses into a nested required complex property and fills unsampled required scalars", () => {
    ...
    assert.deepEqual(sampleFor(node, [inner, middle]).middle, {
      inner: { id: "sample" },
    });
  });
  ...
  it("names a concrete variant when the required property is a polymorphic base", () => {
    ...
    assert.deepEqual(sampleFor(node, [base, text]).content, {
      kind: "text",
      value: "hello",
    });
  });
  ...
  it("skips a wildcard variant when naming a concrete type", () => {
    ...
    assert.deepEqual(sampleFor(node, [base, custom, text]).content, {
      kind: "text",
    });
  });
  ...
  it("stops at a self-referential required property instead of recursing forever", () => {
    ...
    assert.ok(
      !("child" in sampleFor(node, [node])),
      "self-reference must not be synthesized",
    );
  });
});
...
describe("csharp driver — generated fixtures satisfy generated loaders", () => {
  ...
  it("includes a required complex property that declares no @sample", () => {
    ...
    assert.match(
      rendered,
      /detail-code/,
      "required complex payload must reach the C# fixture",
    );
  });
