...
describe("Python test emitter", () => {
      ...
      coercions: [
        {
          ...
          example: true,
          title: "boolean",
          expansion: { kind: "boolean", default: "{value}" },
        },
        {
          scalar: "string",
          example: "hello",
  ...
  it("substitutes {param} placeholders in factory assertions", () => {
    ...
    assert.match(
      output,
      /instance = FixtureReference\.named\("test", "test"\)/,
    );
  ...
  it("adds Pydantic differential assertions only for native serialization mode", () => {
    ...
    const context = buildBaseTestContext(node, "fixtures", pythonTestOptions);
    const defaultOutput = emitPythonTest({
      ...context,
      classCtx: emptyClassCtx(node),
    });
    ...
    assert.match(
      pydanticOutput,
      /assert instance\.model_dump\(\) == saved_data/,
    );
    assert.match(
      pydanticOutput,
    ...
    assert.match(
      pydanticOutput,
      /assert FixtureReference\.model_validate\(original_data\)\.save\(\) == saved_data/,
    );
    assert.match(
      pydanticOutput,
      /assert FixtureReference\.model_validate_json\(json_data\)\.save\(\) == saved_data/,
    );
    assert.match(
      pydanticOutput,
      /FixtureReference\.model_validate_strings\(original_data\)/,
    );
    assert.match(
      pydanticOutput,
