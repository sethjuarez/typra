...
describe("Swift polymorphic enums", () => {
  ...
  it("loads explicit named collections from maps, lists, and scalar shorthand", () => {
    ...
    assert.match(source, /var object = value as\? \[String: Any\] \?\? \["source": value\]/);
    assert.match(source, /if object\["name"\] == nil \{ object\["name"\] = name \}/);
    assert.doesNotMatch(source, /object\["name"\] = name\s+return/);
  ...
  it("applies scalar coercions before polymorphic dispatch", () => {
    ...
    assert.match(source, /if let scalar = data as\? String \{\s+var object: \[String: Any\] = \[:\]\s+object\["kind"\] = "string"\s+object\["example"\] = scalar\s+return try load\(object, context: context\)/);
  });
});
