...
const SUPPORTED_NATIVE_SERIALIZATION = new Map<
  string,
  Set<NativeSerializationMode>
>([
...
function supportedModesFor(targetType: string): Set<NativeSerializationMode> {
  return (
    SUPPORTED_NATIVE_SERIALIZATION.get(targetType.toLowerCase().trim()) ??
    new Set(["none"])
  );
}

export function validateNativeSerializationTargets(
  targets: readonly Pick<EmitTarget, "type" | "native-serialization">[],
): string[] {
  const errors: string[] = [];
