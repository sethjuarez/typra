...
export function emitSwiftTests(
  ctx: BaseTestContext & {
    moduleName: string;
  ...
  ctx.coercions.forEach((coercion, index) => {
    ...
    const child =
      expansion && ctx.node.discriminator
...
function emitExampleTest(
  ...
  if (nativeSerialization === "codable") {
    lines.push(
      `    try assertCodableMatches(${typeName}.self, sourceJson: json, message: "example ${index + 1}")`,
    );
  }
  lines.push("  }");
  lines.push("");
...
function emitCodableAssertions(lines: string[]): void {
  lines.push(
    "  private func assertCodableMatches<T: TypraModel & Codable>(_ type: T.Type, sourceJson: String, message: String) throws {",
  );
  lines.push("    let sourceData = Data(sourceJson.utf8)");
  lines.push(
    "    let typraLoaded = try T.load(try TypraRuntime.jsonObject(from: sourceJson, typeName: String(describing: T.self)), context: LoadContext())",
  );
  lines.push(
    "    let codableLoaded = try JSONDecoder().decode(T.self, from: sourceData)",
  );
  lines.push("    let encoder = JSONEncoder()");
  lines.push("    encoder.outputFormatting = [.sortedKeys]");
  lines.push(
    "    let codableObject = try JSONSerialization.jsonObject(with: encoder.encode(codableLoaded), options: [])",
  );
  lines.push(
    "    let codableEncodedLoaded = try T.load(codableObject, context: LoadContext())",
  );
  lines.push(
    '    XCTAssertEqual(try TypraRuntime.jsonString(from: try codableEncodedLoaded.save(SaveContext())), try TypraRuntime.jsonString(from: try typraLoaded.save(SaveContext())), message + " Codable encode")',
  );
  lines.push(
    '    XCTAssertEqual(try TypraRuntime.jsonString(from: try codableLoaded.save(SaveContext())), try TypraRuntime.jsonString(from: try typraLoaded.save(SaveContext())), message + " Codable decode")',
  );
  lines.push("  }");
  lines.push("");
}
...
function emitStructuredValidations(
  ...
  for (const prop of node.properties) {
    ...
    if (prop.isCollection) {
      ...
      if (count !== undefined) {
        lines.push(
          `    XCTAssertEqual(${requiredAccessor(accessor, prop.isOptional)}.count, ${count})`,
        );
      }
      continue;
    }
    ...
    if (
      ...
      emitNestedValidation(
        lines,
        requiredAccessor(accessor, prop.isOptional),
        value,
        prop,
      );
    }
...
function matchingPolymorphicChild(
  node: TypeNode,
  sample: Record<string, any>,
): TypeNode | undefined {
  const discriminatorValue = node.discriminator
    ? sample[node.discriminator]
    : undefined;
  return (
    node.childTypes.find(
      (child) => polymorphicChildValue(node, child) === discriminatorValue,
    ) ?? defaultPolymorphicChild(node)
  );
}

function defaultPolymorphicChild(node: TypeNode): TypeNode | undefined {
  return node.childTypes.find(
    (child) => polymorphicChildValue(node, child) === "*",
  );
}

function polymorphicChildValue(node: TypeNode, child: TypeNode): unknown {
  return (
    child.properties.find((prop) => prop.name === node.discriminator)
      ?.defaultValue ?? "*"
  );
}
...
function emitPropertyAssertion(
  ...
  if (prop?.enumName) {
    ...
    if (expected.startsWith('"')) {
      lines.push(
        `${indent}XCTAssertEqual(${valueAccessor}.rawValue, ${expected})`,
      );
    } else {
      ...
      lines.push(
        `${indent}XCTAssertEqual(${valueAccessor}, ${normalizedExpected})`,
      );
    }
    return;
  }
  ...
  if (prop && !prop.isScalar && prop.type) {
    ...
    if (nestedKey) {
      ...
      if (prop.type.childTypes.length > 0) {
        lines.push(
          `${indent}XCTAssertEqual(try ${valueAccessor}.save()[${swiftStringLiteral(nestedKey)}] as? String, ${expected})`,
        );
      } else {
        const nestedProp = prop.type.properties.find(
          (candidate) => candidate.name === nestedKey,
        );
        const nestedAccessor = `${valueAccessor}.${swiftPropertyName(nestedKey)}`;
        lines.push(
          `${indent}XCTAssertEqual(${requiredAccessor(nestedAccessor, nestedProp?.isOptional ?? false)}, ${expected})`,
        );
      }
  ...
  lines.push(
    `${indent}XCTAssertEqual(${requiredAccessor(accessor, prop?.isOptional ?? false)}, ${expected})`,
  );
}
...
function coercionProperty(node: TypeNode): string | undefined {
  for (const coercion of node.coercions) {
    const inputAssignment = Object.entries(coercion.expansion).find(
      ([, value]) => value === "{value}",
    );
...
function findProperty(node: TypeNode, key: string): PropertyNode | undefined {
  return node.properties.find(
    (candidate) =>
      ...
      candidate.knownAs.some(
        (mapping) => swiftPropertyName(mapping.name) === swiftPropertyName(key),
      ),
  );
}
