...
export const generateSwift = async (
  ...
  if ((options?.callableVectors?.vectors.length ?? 0) > 0) {
    ...
    if (conformanceEntries.length > 0) {
      await emitSwiftGeneratedFile(
        context,
        swiftFileName("VectorConformance"),
        emitSwiftVectorConformanceEntrypoint(conformanceEntries),
        sourceRoot,
        outputDir,
      );
    }
...
function emitSwiftVectorConformanceEntrypoint(
  ...
  // eligible vectors so the `<Seam>` constraint enforces the WHOLE seam.
  ...
  seamNames.forEach((seamName, seamIndex) => {
    const seam = swiftTypeName(seamName);
    const seamEntries = [...bySeam.get(seamName)!].sort((left, right) =>
      (left.vector.name ?? "").localeCompare(right.vector.name ?? ""),
    );
    const fn = `run${seam}Conformance`;

    lines.push(
      `/// Typed @vector conformance for ${seam}. Pass your real \`${seam}\` impl; the`,
      `/// \`S: ${seam}\` constraint makes the compiler prove every op is implemented.`,
      `public func ${fn}<S: ${seam}>(_ seam: S) async throws {`,
    );

    seamEntries.forEach((entry, index) => {
      const method = swiftFunctionName(entry.operation);
      const paramNames = Object.keys(entry.params);
      const label = entry.vector.name ?? `vector ${index}`;
      const awaitPrefix = entry.sync ? "" : "await ";
      const inputLiteral = swiftPayloadLiteral(
        JSON.stringify(entry.vector.input ?? {}),
      );
      const call = `try ${awaitPrefix}seam.${method}(${paramNames
        .map((paramName) => {
          const local = swiftPropertyName(paramName);
          return `${local}: ${local}`;
        })
      ...
      lines.push("  do {");
      lines.push(`    let inputData = ${inputLiteral}.data(using: .utf8)!`);
      lines.push(
      ...
      lines.push(
        `      throw SeamConformanceError(${swiftStringLiteral(
          `${label}: failed to parse embedded vector input`,
        )})`,
      );
      lines.push("    }");
      for (const paramName of paramNames) {
      ...
      if (entry.vector.expectedError !== undefined) {
        ...
        lines.push("    }");
        lines.push("    guard let caughtError = caught else {");
        lines.push(
          `      throw SeamConformanceError(${swiftStringLiteral(
        ...
        if (typeof entry.vector.expectedError === "string") {
          lines.push(
            `    guard String(describing: caughtError).contains(${swiftStringLiteral(
        ...
        lines.push(`    let actual = ${call}`);
        if (entry.vector.expected !== undefined) {
          const expectedLiteral = swiftPayloadLiteral(
            JSON.stringify(entry.vector.expected),
          );
          lines.push(
            `    let expectedData = ${expectedLiteral}.data(using: .utf8)!`,
          );
          lines.push(
