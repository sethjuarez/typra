...
function emitSwiftVectorConformanceTest(
  ...
  const lines: string[] = [
    ...
    "  }",
    "",
    "  private func runVector(contract: String, operation: String, vector: [String: Any], sync: Bool) async -> Bool {",
  ...
  model.vectors.forEach((entry, index) => {
    ...
    lines.push(
      ...
      "      return",
      "    }",
      `    _ = await runVector(contract: ${JSON.stringify(
        entry.contract,
      )}, operation: ${JSON.stringify(
        entry.operation,
      )}, vector: vector, sync: ${entry.sync ? "true" : "false"})`,
      "  }",
      "",
    );
  });
