...
function emitSwiftVectorConformanceTest(
  ...
  return [
    ...
    "// must be registered with the `sync:` initializer — an `asynchronous:` adapter is",
    "// a hard failure. An async-capable operation (the default) accepts either form.",
    ...
    '    let sync = entry["sync"] as? Bool ?? false',
    "",
    ...
    ...(hasRequires
      ? [
          ...
          "    if !requires.isEmpty {",
          `      let capabilities = ${adapterEnum}.capabilities()`,
    ...
    `    let vectorWaivers = ${adapterEnum}.waivers()`,
    ...
    "  private func evaluateVector(_ adapter: VectorAdapter, _ vector: [String: Any],",
    ...
    "  private func invokeAdapter(_ adapter: VectorAdapter, _ input: Any?,",
    ...
    "  }",
    "",
    "  private func vectorMismatch(_ vectorId: String, _ expected: Any?,",
