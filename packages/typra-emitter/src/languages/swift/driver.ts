...
function emitSwiftVectorConformanceTest(
  ...
  return [
    ...
    "//",
    "// Adapter contract: `invoke` is `async throws`, so an adapter may await real",
    "// async runtime work on XCTest's own runtime; a synchronous adapter is a",
    "// subtype and is accepted unchanged. Each vector performs exactly one awaited",
    "// invocation and must not spawn its own concurrency, keeping conformance",
    "// deterministic.",
    ...
    "  let invoke: (Any?, VectorContext) async throws -> Any?",
    "  let normalize: ((Any?, VectorContext) -> Any?)?",
    "  init(",
    "    _ invoke: @escaping (Any?, VectorContext) async throws -> Any?,",
    ...
    "  func testVectorConformance() async throws {",
    ...
    "        continue",
    "      }",
    "      if !(await runVector(entry)) { failures += 1 }",
    "    }",
    ...
    "  }",
    "",
    "  private func runVector(_ entry: [String: Any]) async -> Bool {",
    ...
    "        do {",
    "          _ = try await adapter.invoke(input, ctx)",
    ...
    "        }",
    "      }",
    "      let observed = try await adapter.invoke(input, ctx)",
