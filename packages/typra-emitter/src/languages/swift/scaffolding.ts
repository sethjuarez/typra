...
export function emitSwiftRuntime(
  moduleName: string,
...
export function emitSwiftProtocolScaffolds(
  protocols: TypeNode[],
  moduleName: string,
): string {
  ...
  for (const protocol of protocols) {
    ...
    for (const method of protocol.methods) {
      ...
      if (returnType !== "Void") {
        lines.push(
          `        throw TypraRuntimeError.unsupported(${swiftStringLiteral(`${protocolName}.${method.name} is a compile-only protocol scaffold.`)})`,
        );
      } else {
        lines.push(
          `        throw TypraRuntimeError.unsupported(${swiftStringLiteral(`${protocolName}.${method.name} is a compile-only protocol scaffold.`)})`,
    ...
    lines.push("    }");
    lines.push(
      `    let ${swiftPropertyName(protocolName)}: ${protocolName} = ${className}()`,
    );
