...
export function emitSwiftFile(
  ...
  const polymorphicDefaultCases = new Map<
    string,
    SwiftPolymorphicDefaultCase | null
  >(
    augmentedUniverse
      ...
      .map((type) => {
        ...
        const fallbackIsDeclaredVariant = fallback
          ? dispatch.variants.some(
              (variant) => variant.typeName.name === fallback.typeName.name,
            )
          : false;
        const closedDefault =
          isClosedPolymorphicDispatch(dispatch) && dispatch.variants[0]
            ? {
                typeName: dispatch.variants[0].typeName.name,
                carriesRaw: false,
              }
            : null;
        return [
          ...
          fallback && !fallback.isSelfReference
            ? {
                typeName: fallback.typeName.name,
                carriesRaw: !fallbackIsDeclaredVariant,
              }
            : closedDefault,
        ] as const;
      }),
  ...
  const fileTypes = augmentedUniverse.filter((type) =>
    emittedKeys.has(typeKey(type)),
  );
...
function emitType(
  ...
  polymorphicTypeNames: Set<string>,
  allTypes: TypeDecl[],
  polymorphicDefaultCases: ReadonlyMap<
    string,
    SwiftPolymorphicDefaultCase | null
  >,
...
function emitEnum(enumDef: EnumDef, lines: string[]): void {
  ...
  if (enumDef.isOpen) {
    lines.push(
      `public struct ${typeName}: RawRepresentable, Equatable, Hashable, Codable {`,
    );
    lines.push("  public let rawValue: String");
    lines.push("  public init(rawValue: String) { self.rawValue = rawValue }");
    for (const value of enumDef.values) {
      lines.push(
        `  public static let ${swiftPropertyName(value)} = ${typeName}(rawValue: ${swiftStringLiteral(value)})`,
      );
    }
  ...
  for (const value of enumDef.values) {
    lines.push(
      `  case ${swiftPropertyName(value)} = ${swiftStringLiteral(value)}`,
    );
  }
  emitEnumParseMethod(enumDef, lines, typeName);
  lines.push("}");
...
function emitEnumParseMethod(
  enumDef: EnumDef,
  lines: string[],
  typeName: string,
): void {
  lines.push(
    `  public static func parse(_ value: String) throws -> ${typeName} {`,
  );
  lines.push("    switch value {");
  for (const value of enumDef.values) {
    lines.push(
      `    case ${swiftStringLiteral(value)}: return ${enumDef.isOpen ? `.${swiftPropertyName(value)}` : `.${swiftPropertyName(value)}`}`,
    );
  }
  for (const [canonical, aliases] of Object.entries(enumDef.parseAliases)) {
    for (const alias of aliases) {
      lines.push(
        `    case ${swiftStringLiteral(alias)}: return .${swiftPropertyName(canonical)}`,
      );
    }
  }
  if (enumDef.isOpen) {
    ...
    lines.push(
      `    default: throw TypraRuntimeError.invalidEnum(type: ${swiftStringLiteral(typeName)}, value: value)`,
    );
  }
  lines.push("    }");
  lines.push("  }");
...
function emitPolymorphicEnum(
  ...
  const dispatch = type.polymorphicDispatch!;
  const indirect = isRecursivePolymorphicEnum(type, allTypes)
    ? "indirect "
    : "";
  const conformances = ["TypraModel"];
  if (nativeSerialization === "codable") conformances.push("Codable");
  lines.push(
    `public ${indirect}enum ${typeName}: ${conformances.join(", ")} {`,
  );
  const fallback =
  ...
  for (const variant of dispatch.variants) {
    ...
    lines.push(
      `  case ${swiftPropertyName(variant.typeName.name)}(${childName})`,
    );
  }
  if (
    fallback &&
    ...
    lines.push(
      `  case ${swiftPropertyName(fallback.typeName.name)}(${swiftTypeName(fallback.typeName.name)}, [String: Any])`,
    );
  }
  ...
  lines.push("");
  lines.push(
    `  public static func load(_ data: Any, context: LoadContext = LoadContext()) throws -> ${typeName} {`,
  );
  lines.push(
    `    ${type.load.coercions.length > 0 ? "var" : "let"} normalizedData: Any = data`,
  );
  ...
  for (const variant of dispatch.variants) {
    lines.push(
      `    case ${swiftStringLiteral(variant.value)}: return .${swiftPropertyName(variant.typeName.name)}(try ${swiftTypeName(variant.typeName.name)}.load(normalizedData, context: context))`,
    );
  }
  if (fallback) {
    lines.push(
      `    default: return .${swiftPropertyName(fallback.typeName.name)}(try ${swiftTypeName(fallback.typeName.name)}.load(normalizedData, context: context), object)`,
    );
  } else if (isClosed) {
    lines.push(
      `    default: throw TypraRuntimeError.unknownDiscriminator(type: ${swiftStringLiteral(typeName)}, field: ${swiftStringLiteral(dispatch.discriminatorField)}, value: discriminator)`,
    );
  } else {
  ...
  for (const variant of dispatch.variants) {
    lines.push(
      `    case .${swiftPropertyName(variant.typeName.name)}(let value): return try value.save(context)`,
    );
  }
  if (
    fallback &&
    ...
    lines.push(
      `    case .${swiftPropertyName(fallback.typeName.name)}(let value, let raw):`,
    );
...
function isRecursivePolymorphicEnum(
  type: TypeDecl,
  allTypes: TypeDecl[],
): boolean {
  const targetName = type.typeName.name;
  const byName = new Map(
    allTypes.map((candidate) => [candidate.typeName.name, candidate]),
  );
  ...
  const relatedTypes = [
    ...type.polymorphicDispatch!.variants.map(
      (variant) => variant.typeName.name,
    ),
...
function emitProtocol(type: TypeDecl, lines: string[]): void {
  ...
  for (const method of type.methods) {
    const params = Object.entries(method.params)
      .map(
        ([name, typeName]) =>
          `${swiftPropertyName(name)}: ${swiftType(typeName)}`,
      )
      .join(", ");
    const asyncToken = method.sync ? "" : " async";
    const returnType = swiftType(method.returns);
    lines.push(
      `  func ${swiftFunctionName(method.name)}(${params})${asyncToken} throws${returnType === "Void" ? "" : ` -> ${returnType}`}`,
    );
  }
  lines.push("}");
  lines.push("");
...
function emitStruct(
  ...
  for (const field of type.fields) {
    const defaultValue = swiftFieldDefaultValue(
      field,
      polymorphicTypeNames,
      polymorphicDefaultCases,
    );
    lines.push(
      `  public var ${swiftPropertyName(field.name)}: ${swiftFieldType(field, polymorphicTypeNames)} = ${defaultValue}`,
    );
  }
  lines.push("");
  lines.push(
    `  public init(${type.fields
      .map((field) => {
        const defaultValue = swiftFieldDefaultValue(
          field,
          polymorphicTypeNames,
  ...
  for (const field of type.fields) {
    lines.push(
      `    self.${swiftPropertyName(field.name)} = ${swiftPropertyName(field.name)}`,
    );
  }
  lines.push("  }");
  lines.push("");
  ...
  for (const factory of type.factories) {
    ...
    const params = Object.entries(factory.params)
      .map(
        ([name, paramType]) =>
          `${swiftPropertyName(name)}: ${swiftType(paramType)}`,
      )
      .join(", ");
    lines.push(
      `  public static func ${swiftFunctionName(factory.name)}(${params}) -> ${typeName} {`,
    );
...
function emitCodableDelegation(typeName: string, lines: string[]): void {
  ...
  lines.push(
    "    self = try Self.load(try TypraCodableValue(from: decoder).anyValue)",
  );
  lines.push("  }");
  lines.push("");
...
function emitLoad(
  ...
  lines.push(
    `    let object = try TypraRuntime.object(data, typeName: ${swiftStringLiteral(typeName)})`,
  );
  ...
  for (const assignment of type.load.assignments) {
    ...
    if (
      ...
      lines.push(
        `      throw TypraRuntimeError.unsupported(context.at(${swiftStringLiteral(assignment.sourceName)}).path + ": missing required field")`,
      );
      lines.push("    }");
    }
    lines.push(
      `    if let value = object[${swiftStringLiteral(assignment.sourceName)}] {`,
    );
    lines.push(
      `      instance.${swiftPropertyName(assignment.fieldName)} = ${swiftLoadExpression(assignment.category, assignment.enumName, assignment.fieldName, "value", polymorphicTypeNames, collectionHelper)}`,
    );
    lines.push("    }");
    if (
      ...
      lines.push("    else {");
      lines.push(
        `      instance.${swiftPropertyName(field.name)} = ${swiftLiteralDefault(field)}`,
      );
      lines.push("    }");
    }
  }
...
function emitNamedCollectionLoadHelper(
  ...
  lines.push(
    `  private static func ${methodName}(_ data: Any, context: LoadContext) throws -> [${elementType}] {`,
  );
  ...
  lines.push("    }");
  lines.push(
    `    let values = try TypraRuntime.dictionary(data, field: ${swiftStringLiteral(helper.propertyName)})`,
  );
  ...
  lines.push("      } else {");
  lines.push(
    `        itemData = try ${elementType}.load(value, context: context).save()`,
  );
  lines.push("      }");
  lines.push('      itemData["name"] = name');
  lines.push(
    `      return try ${elementType}.load(itemData, context: context.at(name))`,
  );
  lines.push("    }");
  lines.push("  }");
}
...
function emitNamedCollectionSaveHelper(
  ...
  if (!polymorphicTypeNames.has(helper.elementTypeName.name)) {
    const shorthandProperty =
      helper.innerFields.length === 1 ? helper.innerFields[0] : undefined;
    if (shorthandProperty) {
      lines.push(
        `      if context.useShorthand, itemData.count == 1, let value = itemData[${swiftStringLiteral(shorthandProperty)}] {`,
      );
...
function addNamedCollectionFields(types: TypeDecl[]): TypeDecl[] {
  ...
  const namedElementKeys = new Set(
    types
      .flatMap((type) => type.collectionHelpers)
      .filter((helper) =>
        supportsNamedCollectionHelper(helper, polymorphicTypeNames),
      )
      .map(
        (helper) =>
          `${helper.elementTypeName.namespace}.${helper.elementTypeName.name}`,
      ),
  );
  ...
  return types.map((type) => {
    if (
      !namedElementKeys.has(typeKey(type)) ||
  ...
  for (const assignment of coercion.assignments) {
    ...
    const expr = assignment.isInput
      ? swiftLoadExpression(
          field?.category ?? {
            kind: "scalar",
            scalarType: coercion.scalarType,
          },
          field?.enumName ?? null,
          fieldName,
          "scalar",
...
function emitSave(
  ...
  for (const assignment of type.save.assignments) {
    ...
    if (shouldOmitAbsentOnSave(assignment)) {
      ...
      lines.push(
        `      result[${swiftStringLiteral(assignment.targetName)}] = ${swiftSaveExpression(assignment.category, assignment.enumName, "value", polymorphicTypeNames, collectionHelper)}`,
      );
      lines.push("    }");
    } else {
      lines.push(
        `    result[${swiftStringLiteral(assignment.targetName)}] = ${serialized}`,
      );
    }
  }
  lines.push("    return result");
...
function emitToWire(
  ...
  for (const mapping of type.wire!.mappings) {
    const collectionHelper = type.collectionHelpers.find(
      (helper) =>
        helper.propertyName === mapping.fieldName &&
        supportsNamedCollectionHelper(helper, polymorphicTypeNames),
    );
...
function emitWireField(
  ...
  for (const [provider, wireName] of Object.entries(mapping.wireNames)) {
    lines.push(
      `    case ${swiftStringLiteral(provider)}: ${target} = ${swiftStringLiteral(wireName)}`,
    );
  }
  ...
  if (mapping.isOptional) {
    lines.push(
      `    if let wireKey = ${target}, let value = ${source} { result[wireKey] = ${swiftSaveExpression(mapping.category, null, "value", polymorphicTypeNames, collectionHelper)} }`,
    );
  } else {
    lines.push(
      `    if let wireKey = ${target} { result[wireKey] = ${swiftSaveExpression(mapping.category, null, source, polymorphicTypeNames, collectionHelper)} }`,
    );
  }
}
...
function emitJsonYamlMethods(typeName: string, lines: string[]): void {
  lines.push("");
  lines.push(
    `  public static func fromJSON(_ json: String, context: LoadContext = LoadContext()) throws -> ${typeName} {`,
  );
  lines.push(
    `    return try load(TypraRuntime.jsonObject(from: json, typeName: ${swiftStringLiteral(typeName)}), context: context)`,
  );
  lines.push("  }");
  lines.push("");
  lines.push(
  ...
  lines.push("");
  lines.push(
    `  public static func fromYAML(_ yaml: String, context: LoadContext = LoadContext()) throws -> ${typeName} {`,
  );
  lines.push(
    `    return try load(TypraRuntime.yamlObject(from: yaml, typeName: ${swiftStringLiteral(typeName)}), context: context)`,
  );
  lines.push("  }");
  lines.push("");
  lines.push(
...
function emitMethodStub(method: MethodStubDecl, lines: string[]): void {
  ...
  lines.push("");
  lines.push(
    `  public func ${swiftFunctionName(method.name)}(${params})${asyncToken} throws${returnType === "Void" ? "" : ` -> ${returnType}`} {`,
  );
  lines.push(
    `    throw TypraRuntimeError.unsupported(${swiftStringLiteral(`${method.name} must be implemented by hand-authored code.`)})`,
  );
  lines.push("  }");
}

function swiftFieldType(
  field: FieldDecl,
  polymorphicTypeNames: Set<string>,
): string {
  const base = swiftCategoryType(
    field.category,
    field.enumName,
    polymorphicTypeNames,
  );
...
function swiftRecordValueType(valueType: string | undefined): string {
  ...
  return mapped === "Any" && valueType !== "any" && valueType !== "unknown"
    ? swiftTypeName(valueType)
    : mapped;
}
...
function swiftDefaultValue(
  ...
  if (field.enumName)
    return `(try! ${swiftTypeName(field.enumName)}.parse(${swiftStringLiteral(field.allowedValues[0] ?? "")}))`;
...
function swiftLiteralForField(
  field: FieldDecl | undefined,
...
function swiftLoadExpression(
  ...
  switch (category.kind) {
    case "scalar":
      if (enumName)
        return `try ${swiftTypeName(enumName)}.parse(try TypraRuntime.string(${valueExpr}, field: ${swiftStringLiteral(fieldName)}))`;
...
function swiftRecordValueLoadExpression(
  valueType: string,
  valueExpr: string,
  fieldName: string,
): string {
...
function scalarLoadExpression(
  ...
  switch (scalarType) {
    case "string":
      return `try TypraRuntime.string(${valueExpr}, field: ${swiftStringLiteral(fieldName)})`;
    case "boolean":
      return `try TypraRuntime.bool(${valueExpr}, field: ${swiftStringLiteral(fieldName)})`;
    case "int32":
      return `try TypraRuntime.int32(${valueExpr}, field: ${swiftStringLiteral(fieldName)})`;
    case "int64":
      return `try TypraRuntime.int64(${valueExpr}, field: ${swiftStringLiteral(fieldName)})`;
    case "integer":
      return `try TypraRuntime.int(${valueExpr}, field: ${swiftStringLiteral(fieldName)})`;
    case "float32":
      return `try TypraRuntime.float(${valueExpr}, field: ${swiftStringLiteral(fieldName)})`;
    case "float64":
    case "float":
    case "number":
    case "numeric":
      return `try TypraRuntime.double(${valueExpr}, field: ${swiftStringLiteral(fieldName)})`;
    case "object":
    case "dictionary":
      return `try TypraRuntime.dictionary(${valueExpr}, field: ${swiftStringLiteral(fieldName)})`;
    case "array":
      return `try TypraRuntime.array(${valueExpr}, field: ${swiftStringLiteral(fieldName)})`;
    default:
      return valueExpr;
  }
}
  ...
  switch (category.kind) {
    ...
    case "dict":
      if (
        ...
        !isSwiftScalarRecordValue(category.valueType)
      ) {
