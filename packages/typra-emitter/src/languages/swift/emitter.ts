...
export function emitSwiftFile(
  ...
  const types = flattenInheritance(file.types, declarationUniverse);
  const lines: string[] = [
...
function emitPolymorphicEnum(type: TypeDecl, lines: string[], allTypes: TypeDecl[]): void {
  ...
  for (const coercion of type.load.coercions) {
    ...
    lines.push("      var object: [String: Any] = [:]");
    for (const assignment of coercion.assignments) {
      const value = assignment.isInput
        ? "scalar"
        : swiftStringLiteral(assignment.literalValue ?? "");
      lines.push(`      object[${swiftStringLiteral(assignment.fieldName)}] = ${value}`);
    }
    lines.push("      return try load(object, context: context)");
    lines.push("    }");
  }
...
function emitStruct(type: TypeDecl, lines: string[], visitor: ExprVisitor, polymorphicTypeNames: Set<string>): void {
  ...
  for (const helper of type.collectionHelpers) {
    if (helper.hasNameProperty) {
      ...
      lines.push("");
      emitNamedCollectionSaveHelper(helper, lines);
    }
  }
  lines.push("");
...
function emitNamedCollectionLoadHelper(helper: CollectionHelperDecl, lines: string[]): void {
  const methodName = `load${swiftTypeName(helper.propertyName)}`;
  const elementType = swiftTypeName(helper.elementTypeName.name);
  const shorthandField = helper.innerFields[0] ?? "kind";
  ...
  lines.push(`      var object = value as? [String: Any] ?? [${swiftStringLiteral(shorthandField)}: value]`);
  lines.push('      if object["name"] == nil { object["name"] = name }');
  lines.push(`      return try ${elementType}.load(object, context: context)`);
  lines.push("    }");
  lines.push("  }");
}

function emitNamedCollectionSaveHelper(helper: CollectionHelperDecl, lines: string[]): void {
  ...
  lines.push("      var object = try item.save(context)");
  lines.push('      guard let name = object.removeValue(forKey: "name") as? String else {');
  lines.push(`        throw TypraRuntimeError.unsupported("Cannot save named ${helper.propertyName} item without a name.")`);
  lines.push("      }");
  lines.push("      result[name] = object");
  lines.push("    }");
...
function emitSave(type: TypeDecl, lines: string[], polymorphicTypeNames: Set<string>): void {
  ...
  for (const assignment of type.save.assignments) {
    ...
    const serialized = helper
      ? `try Self.save${swiftTypeName(helper.propertyName)}(${prop}, context: context)`
      : swiftSaveExpression(assignment.category, assignment.enumName, prop, polymorphicTypeNames);
    if (assignment.isOptional) {
      lines.push(`    if let value = ${prop} {`);
      const optionalSerialized = helper
        ? `try Self.save${swiftTypeName(helper.propertyName)}(value, context: context)`
        : swiftSaveExpression(assignment.category, assignment.enumName, "value", polymorphicTypeNames);
      lines.push(`      result[${swiftStringLiteral(assignment.targetName)}] = ${optionalSerialized}`);
      lines.push("    }");
    } else {
