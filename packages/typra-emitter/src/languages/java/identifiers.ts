...
const JAVA_KEYWORDS = new Set([
  ...
  "void", "volatile", "while", "with", "yield", "true", "false", "null", "_",
]);

export function javaIdentifier(name: string, fallback = "value"): string {
  const normalized = name.replace(/[^A-Za-z0-9_$]/g, "_");
  const started = /^[A-Za-z_$]/.test(normalized) ? normalized : `${fallback}_${normalized}`;
...
export function javaEnumMemberName(value: string): string {
  const normalized = value.replace(/[^A-Za-z0-9]+/g, "_").replace(/^(\d)/, "_$1").toUpperCase();
