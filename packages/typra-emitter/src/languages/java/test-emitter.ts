...
export function emitJavaTest(ctx: BaseTestContext, nativeSerialization: JavaNativeSerialization = "none"): string {
  ...
  if (ctx.examples.length > 0) {
    ctx.examples.forEach((example, index) => {
      emitExampleTest(lines, typeName, ctx.node, example, index + 1, nativeSerialization);
    });
...
function emitExampleTest(
  ...
  if (nativeSerialization === "jackson") {
    lines.push(`    assertJacksonMatches(instance${index}, ${jsonData}, ${typeName}.class, "Expected Jackson to match Typra load/save for ${typeName} example ${index}");`);
  }
}
...
function emitAssertions(lines: string[], nativeSerialization: JavaNativeSerialization): void {
  lines.push("");
  ...
  if (nativeSerialization === "jackson") {
    lines.push("");
    lines.push("  private static <T> void assertJacksonMatches(T instance, String sourceJson, Class<T> type, String message) {");
    lines.push("    try {");
    lines.push("      com.fasterxml.jackson.databind.ObjectMapper mapper = new com.fasterxml.jackson.databind.ObjectMapper();");
    lines.push("      String typraJson = (String) type.getMethod(\"toJson\").invoke(instance);");
    lines.push("      String jacksonJson = mapper.writeValueAsString(instance);");
    lines.push("      assertEquals(mapper.readValue(typraJson, Object.class), mapper.readValue(jacksonJson, Object.class), message + \" serialization\");");
    lines.push("      T jacksonLoaded = mapper.readValue(sourceJson, type);");
    lines.push("      String jacksonLoadedJson = (String) type.getMethod(\"toJson\").invoke(jacksonLoaded);");
    lines.push("      assertEquals(mapper.readValue(typraJson, Object.class), mapper.readValue(jacksonLoadedJson, Object.class), message + \" deserialization\");");
    lines.push("    } catch (ReflectiveOperationException | java.io.IOException error) {");
