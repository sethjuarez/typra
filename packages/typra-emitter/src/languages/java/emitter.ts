...
import {
  EDITABLE_SEAM_MARKER,
  hasEditableSeamMarker,
...
export function emitJavaFileContent(
  ...
  for (const enumDef of enums)
    emitEnum(enumDef, lines, false, nativeSerialization);
...
function hasParseAliases(value: {
  parseAliases: Record<string, string[]>;
}): boolean {
  return Object.values(value.parseAliases).some(
...
function emitOpenEnumAliasParsers(fields: FieldDecl[], lines: string[]): void {
  ...
  for (const field of fields) {
    ...
    lines.push(
      `  private static String ${javaAliasParserName(field.enumName)}(String value) {`,
    );
    lines.push("    switch (value) {");
...
function emitType(
  ...
  const extendsClause = type.base
    ? ` extends ${javaTypeName(type.base.name)}`
    : "";
  if (nativeSerialization === "jackson") {
    lines.push(
      `@JsonSerialize(using = ${typeName}.TypraJacksonSerializer.class)`,
    );
    lines.push(
      `@JsonDeserialize(using = ${typeName}.TypraJacksonDeserializer.class)`,
    );
    lines.push("@JsonInclude(JsonInclude.Include.NON_NULL)");
  }
  lines.push(`${modifiers} class ${typeName}${extendsClause} {`);
  lines.push(
    `  public static final String SHORTHAND_PROPERTY = ${type.coercionProperty ? `"${type.coercionProperty}"` : "null"};`,
  );
  lines.push("");

  const inheritedFields = collectInheritedFields(type, allTypes);
  for (const field of type.fields.filter(
    (field) => !inheritedFields.has(field.name),
  )) {
    if (nativeSerialization === "jackson") {
    ...
    lines.push(
      `  public ${javaFieldType(field, polymorphicTypeNames)} ${javaPropertyName(field.name)}${javaDefault(field, polymorphicTypeNames)};`,
    );
  }
  ...
  lines.push("");
  const inheritedDefaults = type.fields.filter(
    (field) =>
  ...
  if (inheritedDefaults.length === 0) {
    ...
    for (const field of inheritedDefaults) {
      lines.push(
        `    this.${javaPropertyName(field.name)} = ${javaLiteral(field.defaultValue!, field.enumName, scalarTypeOf(field))};`,
      );
    }
    lines.push("  }");
  }
  ...
  function collectInheritedFields(
    ...
    let base = type.base
      ? allTypes.find(
          (candidate) => candidate.typeName.name === type.base?.name,
        )
      : undefined;
    while (base) {
      ...
      base = base.base
        ? allTypes.find(
            (candidate) => candidate.typeName.name === base?.base?.name,
          )
        : undefined;
    }
    return fields;
...
function retainsRawPayload(type: TypeDecl): boolean {
  return (
    type.polymorphicDispatch?.defaultVariant?.isSelfReference === true ||
    needsUnknownCarrier(type)
  );
}
...
export function emitJavaUnknownCarrier(
  ...
  for (const assignment of type.load.assignments) {
    lines.push(
      `    result.rawPayload.remove("${escapeJava(assignment.sourceName)}");`,
    );
  }
...
function emitJacksonHelpers(typeName: string, lines: string[]): void {
  lines.push(
    `  public static final class TypraJacksonSerializer extends com.fasterxml.jackson.databind.JsonSerializer<${typeName}> {`,
  );
  lines.push("    @Override");
  lines.push(
    `    public void serialize(${typeName} value, com.fasterxml.jackson.core.JsonGenerator generator, com.fasterxml.jackson.databind.SerializerProvider serializers) throws java.io.IOException {`,
  );
  lines.push(
  ...
  lines.push("");
  lines.push(
    `  public static final class TypraJacksonDeserializer extends com.fasterxml.jackson.databind.JsonDeserializer<${typeName}> {`,
  );
  lines.push("    @Override");
  lines.push(
    `    public ${typeName} deserialize(com.fasterxml.jackson.core.JsonParser parser, com.fasterxml.jackson.databind.DeserializationContext context) throws java.io.IOException {`,
  );
  lines.push(
...
function emitProtocol(type: TypeDecl, lines: string[]): void {
  ...
  for (const method of type.methods) {
    const params = Object.entries(method.params)
      .map(
        ([name, typeName]) =>
          `${javaScalarType(typeName)} ${javaPropertyName(name)}`,
      )
      .join(", ");
    lines.push(
      `  ${javaScalarType(method.returns)} ${javaMethodName(method.name)}(${params});`,
    );
  }
  lines.push("}");
  lines.push("");
...
function emitLoad(
  ...
  lines.push(
    `  public static ${typeName} load(Object input, LoadContext context) {`,
  );
  lines.push(
    "    LoadContext ctx = context == null ? new LoadContext() : context;",
  );
  ...
  if (type.isAbstract) {
    const discriminator =
      type.polymorphicDispatch?.discriminatorField ?? "type";
    if (needsUnknownCarrier(type)) {
      lines.push("    if (data instanceof Map<?, ?>) {");
      lines.push(
        `      return ${javaUnknownCarrierName(type.typeName.name)}.load(data, ctx);`,
      );
      lines.push("    }");
    }
    lines.push(
      `    throw new IllegalArgumentException("Cannot instantiate abstract ${typeName}; expected a matching '${escapeJava(discriminator)}' discriminator.");`,
    );
    lines.push("  }");
    lines.push("");
  ...
  if (type.polymorphicDispatch?.defaultVariant?.isSelfReference) {
    lines.push(
      `    if (map.get("${escapeJava(type.polymorphicDispatch.discriminatorField)}") instanceof String) {`,
    );
...
function emitLoadBaseInto(
  ...
  const typeName = javaTypeName(type.typeName.name);
  lines.push(
    `  static void loadBaseInto(${typeName} result, Map<?, ?> map, LoadContext ctx) {`,
  );
  if (type.base) {
    lines.push(
      `    ${javaTypeName(type.base.name)}.loadBaseInto(result, map, ctx);`,
    );
  }
  for (const assignment of type.load.assignments) {
    ...
    const collectionHelper = type.collectionHelpers.find(
      (helper) =>
    ...
    if (shouldGuardMissingRequiredField(field)) {
      ...
      lines.push(
        `      throw new IllegalArgumentException(ctx.at("${wireName}").path + ": missing required field");`,
      );
      lines.push("    }");
    }
    if (field)
      emitLoadField(field, collectionHelper, lines, polymorphicTypeNames);
  }
  lines.push("  }");
  lines.push("");
}
...
function emitCoercions(
  ...
  const orderedCoercions = [...coercions].sort(
    (left, right) =>
      numericCoercionRank(left.scalarType) -
      numericCoercionRank(right.scalarType),
  );
  for (const coercion of orderedCoercions) {
    ...
    if (isAbstract) {
      ...
      for (const assignment of coercion.assignments) {
        ...
        lines.push(
          `      coerced.put("${escapeJava(assignment.fieldName)}", ${coercionValue(assignment, coercion.scalarType, field)});`,
        );
      }
    ...
    for (const assignment of coercion.assignments) {
      ...
      lines.push(
        `      result.${javaPropertyName(assignment.fieldName)} = ${coercionValue(assignment, coercion.scalarType, field)};`,
      );
    }
    lines.push("      return ctx.processOutput(result);");
    lines.push("    }");
...
function assertDistinctNumericCoercionFamilies(
  typeName: string,
  coercions: CoercionDecl[],
): void {
  for (const [family, matches] of [
    [
      "integral",
      coercions.filter((coercion) => isIntegralCoercion(coercion.scalarType)),
    ],
    [
      "floating-point",
      coercions.filter((coercion) =>
        isBroadNumberCoercion(coercion.scalarType),
      ),
    ],
  ] as const) {
    if (matches.length > 1) {
...
function isIntegralCoercion(scalarType: string): boolean {
  return (
    scalarType === "int32" || scalarType === "int64" || scalarType === "integer"
  );
}
...
function emitPolymorphicDispatch(
  ...
  for (const variant of dispatch.variants) {
    ...
    lines.push(
      `          return ${javaTypeName(variant.typeName.name)}.load(data, ctx);`,
    );
  }
  lines.push("        default:");
  if (dispatch.defaultVariant && !dispatch.defaultVariant.isSelfReference) {
    lines.push(
      `          return ${javaTypeName(dispatch.defaultVariant.typeName.name)}.load(data, ctx);`,
    );
  } else if (isClosed) {
    lines.push(
      `          throw new IllegalArgumentException("Unknown ${typeName} discriminator field '${escapeJava(dispatch.discriminatorField)}' value: " + discriminatorString);`,
    );
  } else {
    lines.push("          break;");
  }
...
function emitLoadField(
  ...
  switch (field.category.kind) {
    case "scalar":
      lines.push(
        `      result.${name} = ${loadScalar(`map.get("${wireName}")`, field.category.scalarType, field.isOpenEnum ? null : field.enumName, openEnumAliasParser(field))};`,
      );
      break;
    case "complex":
      lines.push(
        `      result.${name} = ${javaTypeName(field.category.typeName)}.load(map.get("${wireName}"), ctx.at("${wireName}"));`,
      );
      break;
    case "collection_scalar":
      lines.push(`      result.${name} = new ArrayList<>();`);
      lines.push(
        `      if (map.get("${wireName}") instanceof Iterable<?> values) {`,
      );
      lines.push("        for (Object item : values) {");
      lines.push(
        `                      result.${name}.add(${loadScalar("item", field.category.scalarType, field.isOpenEnum ? null : field.enumName, openEnumAliasParser(field))});`,
      );
      lines.push("        }");
      lines.push("      }");
      break;
    case "collection_complex":
      ...
      if (collectionHelper?.hasNameProperty) {
        lines.push(
          `      if (map.get("${wireName}") instanceof Map<?, ?> values) {`,
        );
        lines.push("        for (Map.Entry<?, ?> entry : values.entrySet()) {");
        lines.push("          if (entry.getValue() instanceof Iterable<?>) {");
        lines.push(
          `            throw new IllegalArgumentException(ctx.at("${wireName}").at(String.valueOf(entry.getKey())).path + ": invalid named collection entry category array");`,
        );
        lines.push("          }");
        ...
        lines.push(
          ...emitJavaEntryShorthandArms(collectionHelper, "            "),
        );
        lines.push("          }");
        lines.push(
        ...
        lines.push(
          `          ${javaTypeName(field.category.typeName)} item = ${javaTypeName(field.category.typeName)}.load(itemData, ctx.at("${wireName}").at(String.valueOf(entry.getKey())));`,
        );
        lines.push(`          result.${name}.add(item);`);
        lines.push("        }");
        lines.push(
          `      } else if (map.get("${wireName}") instanceof Iterable<?> values) {`,
        );
      } else {
        lines.push(
          `      if (map.get("${wireName}") instanceof Iterable<?> values) {`,
      ...
      lines.push("        for (Object item : values) {");
      lines.push(
        `          result.${name}.add(${javaTypeName(field.category.typeName)}.load(item, ctx.at("${wireName}").atIndex(${name}Index++)));`,
      );
      lines.push("        }");
      lines.push("      }");
      break;
    case "dict":
      lines.push(
        `      if (map.get("${wireName}") instanceof Map<?, ?> dict) {`,
      );
      if (!field.category.valueType || field.category.valueType === "unknown") {
        ...
        lines.push(
          `          result.${name}.put(String.valueOf(entry.getKey()), ${javaDictValueLoad("entry.getValue()", field.category.valueType, `ctx.at("${wireName}").at(String.valueOf(entry.getKey()))`)});`,
        );
        lines.push("        }");
      }
      lines.push("      }");
 ...
 */
function emitJavaEntryShorthandArms(
  ...
  const lines: string[] = [
    `${indent}Object shorthandValue = entry.getValue();`,
  ];
  let first = true;
  for (const entryCase of cases) {
    ...
    for (const a of entryCase.assignments) {
      lines.push(
        `${indent}  itemData.put("${a.fieldName}", ${javaShorthandLiteral(a.literalValue)});`,
      );
    }
  }
  if (!first) lines.push(`${indent}}`);
  lines.push(
    `${indent}itemData.put("${shorthand.valueField}", shorthandValue);`,
  );
  return lines;
}
...
function javaScalarValueCheck(scalarType: string): string | null {
  ...
  if (isStringEncodedScalar(scalarType))
    return "shorthandValue instanceof String";
...
function emitSave(
  ...
  lines.push(
    `    ${javaTypeName(type.typeName.name)} obj = ctx.processObject(this);`,
  );
  if (retainsRawPayload(type)) {
    lines.push(
  ...
  for (const field of type.save.assignments) {
    ...
    emitSaveField(field, collectionHelper, lines, polymorphicTypeNames);
  }
  lines.push("  }");
  lines.push("");
}
  ...
  switch (category.kind) {
    case "scalar":
      if (shouldOmitAbsentOnSave(field, "always-check")) {
        lines.push(
          `    if (obj.${propertyName} != null) result.put("${wireName}", ${enumName ? `obj.${propertyName}.value` : `serializeScalar(obj.${propertyName})`});`,
        );
      } else {
        lines.push(
          `    result.put("${wireName}", ${enumName ? `obj.${propertyName}.value` : `serializeScalar(obj.${propertyName})`});`,
        );
      }
      break;
    case "dict":
      if (
        category.valueType &&
        category.valueType !== "unknown" &&
        !JAVA_TYPE_MAP[category.valueType]
      ) {
        if (shouldOmitAbsentOnSave(field, "always-check"))
          lines.push(`    if (obj.${propertyName} != null) {`);
        lines.push("      Map<String, Object> items = new LinkedHashMap<>();");
        lines.push(
          `      for (Map.Entry<String, ${javaDictValueType(category.valueType)}> entry : obj.${propertyName}.entrySet()) items.put(entry.getKey(), entry.getValue().save(ctx));`,
        );
        ...
        if (shouldOmitAbsentOnSave(field, "always-check")) {
          lines.push(
            `    if (obj.${propertyName} != null) result.put("${wireName}", serializeScalar(obj.${propertyName}));`,
          );
        } else {
          lines.push(
            `    result.put("${wireName}", serializeScalar(obj.${propertyName}));`,
          );
        }
      }
      break;
    case "complex":
      if (shouldOmitAbsentOnSave(field, "always-check")) {
        lines.push(
          `    if (obj.${propertyName} != null) result.put("${wireName}", obj.${propertyName}.save(ctx));`,
        );
      } else {
        lines.push(
          `    result.put("${wireName}", obj.${propertyName}.save(ctx));`,
        );
      }
      break;
    case "collection_scalar":
      if (enumName) {
        ...
        lines.push(
          `      for (${javaEnumTypeName(enumName)} item : obj.${propertyName}) items.add(item.value);`,
        );
        lines.push(`      result.put("${wireName}", items);`);
        if (shouldOmitAbsentOnSave(field, "always-check")) lines.push("    }");
      } else {
        if (shouldOmitAbsentOnSave(field, "always-check")) {
          lines.push(
            `    if (obj.${propertyName} != null) result.put("${wireName}", new ArrayList<>(obj.${propertyName}));`,
          );
        } else {
          lines.push(
            `    result.put("${wireName}", new ArrayList<>(obj.${propertyName}));`,
          );
        }
      }
      break;
    case "collection_complex":
      ...
      if (collectionHelper?.hasNameProperty) {
        ...
        lines.push(
          `      for (${javaTypeName(category.typeName)} item : obj.${propertyName}) {`,
        );
        ...
        lines.push(
          `          if (ctx.useShorthand && ${javaTypeName(category.typeName)}.SHORTHAND_PROPERTY != null && itemData.size() == 1 && itemData.containsKey(${javaTypeName(category.typeName)}.SHORTHAND_PROPERTY)) {`,
        );
        lines.push(
          `            objectItems.put(itemName, itemData.get(${javaTypeName(category.typeName)}.SHORTHAND_PROPERTY));`,
        );
        lines.push("          } else {");
        ...
        lines.push("      List<Object> items = new ArrayList<>();");
        lines.push(
          `      for (${javaTypeName(category.typeName)} item : obj.${propertyName}) items.add(item.save(ctx));`,
        );
        lines.push(`      result.put("${wireName}", items);`);
      }
      if (shouldOmitAbsentOnSave(field, "always-check")) lines.push("    }");
...
function emitToWire(wire: WireDecl, lines: string[]): void {
  ...
  for (const field of wire.mappings) {
    ...
    for (const [provider, name] of Object.entries(field.wireNames)) {
      lines.push(
        `      if (target.equals("${escapeJava(provider)}")) { wireName = "${escapeJava(name)}"; }`,
      );
    }
    lines.push(
      `      if (wireName != null && this.${javaPropertyName(field.fieldName)} != null) result.put(wireName, serializeScalar(this.${javaPropertyName(field.fieldName)}));`,
    );
    lines.push("    }");
  }
  lines.push("    return result;");
...
function emitSerializationHelpers(typeName: string, lines: string[]): void {
  ...
  lines.push("");
  lines.push(
    `  public static ${typeName} fromYaml(String yaml, LoadContext context) {`,
  );
  ...
  lines.push("");
  lines.push(
    `  public static ${typeName} fromJson(String json, LoadContext context) {`,
  );
...
function emitFactory(
  ...
  const params = Object.entries(factory.params)
    .map(([name, type]) => `${javaScalarType(type)} ${javaPropertyName(name)}`)
    .join(", ");
  lines.push(
    `  public static ${typeName} ${javaMethodName(factory.name)}(${params}) {`,
  );
...
function emitMethodDelegation(
  typeName: string,
  method: MethodStubDecl,
  lines: string[],
): void {
  const params = Object.entries(method.params)
    .map(([name, type]) => `${javaScalarType(type)} ${javaPropertyName(name)}`)
    .join(", ");
  lines.push(
    `  public ${javaScalarType(method.returns)} ${javaMethodName(method.name)}(${params}) {`,
  );
  const args = [
    "this",
    ...Object.keys(method.params).map(javaPropertyName),
  ].join(", ");
  if (method.returns === "void") {
    lines.push(
      `    ${typeName}Methods.${javaMethodName(method.name)}(${args});`,
    );
  } else {
    lines.push(
      `    return ${typeName}Methods.${javaMethodName(method.name)}(${args});`,
    );
  }
  lines.push("  }");
  lines.push("");
...
export const JAVA_EDITABLE_SEAM_NOTE =
 ...
 */
export function ensureJavaEditableSeamMarker(
...
export function emitJavaMethodHelper(
  type: TypeDecl,
  packageName: string,
  ...
  for (const method of type.methods) {
    ...
    lines.push(
      `  public static ${javaScalarType(method.returns)} ${javaMethodName(method.name)}(${params}) {`,
    );
    lines.push(
      `    throw new UnsupportedOperationException("Implement ${typeName}.${javaMethodName(method.name)} in ${helperName}.");`,
    );
    lines.push("  }");
    lines.push("");
  }
...
function javaFieldType(
  field: FieldDecl,
  polymorphicTypeNames: Set<string>,
): string {
  switch (field.category.kind) {
    case "scalar":
      return field.enumName && !field.isOpenEnum
        ? javaEnumTypeName(field.enumName)
        : javaScalarType(field.category.scalarType);
    case "complex":
...
function loadScalar(
  ...
  parseAliasFunction: string | null = null,
): string {
  if (enumName) {
...
function coercionValue(
  assignment: CoercionAssignment,
  scalarType: string,
  field?: FieldDecl,
): string {
...
function javaLiteral(
  ...
  if (typeof value === "string") {
    if (enumName)
      return `${javaEnumTypeName(enumName)}.fromValue("${escapeJava(value)}")`;
    return `"${escapeJava(value)}"`;
  }
  if (typeof value === "number" && scalarType)
    return javaNumericLiteral(value, scalarType);
  return String(value);
}
