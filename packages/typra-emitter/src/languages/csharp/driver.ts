...
function emitCSharpVectorConformanceTest(
  ...
  const lines: string[] = [
    ...
    "",
    "    private static void RunVector(int index)",
    "    {",
    ...
    "        else",
    "        {",
    "            var observed = adapter!.Invoke(input, ctx);",
  ...
  model.vectors.forEach((entry, index) => {
    lines.push("    [Fact]");
    lines.push(
      `    public void ${csharpVectorSlug(index, entry)}() => RunVector(${index});`,
    );
    lines.push("");
  });
  lines.push("}");
