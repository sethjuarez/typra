...
/**
 * Escape arbitrary text into the body of a double-quoted C# string literal. Every
 * non-ASCII code point is emitted as `\uXXXX` (or `\UXXXXXXXX` for astral code
 * points), so the embedded payload is pure ASCII and cannot be broken by bidi
 * controls or by characters that would terminate the literal.
 */
function csharpStringLiteralBody(text: string): string {
  let out = "";
...
function emitCSharpVectorConformanceTest(
  ...
  const payloadLiteral = `"${csharpStringLiteralBody(payload)}"`;
  ...
  const lines: string[] = [
    ...
    "//",
    "// Classification is ENFORCED: an operation marked `@sync` (entry[\"sync\"] is",
    "// true) must resolve synchronously — if its adapter returns a Task/ValueTask",
    "// the vector is a hard failure. An async-capable operation (the default) stays",
    "// permissive: a plain value or a Task/ValueTask both pass.",
    ...
    "{",
    `    private const string Payload = ${payloadLiteral};`,
    "",
    "    private static JsonArray LoadVectors() =>",
    "        JsonNode.Parse(Payload) as JsonArray ?? new JsonArray();",
    "",
    ...
    "    }",
    "",
    "    private static async Task RunVector(int index)",
    "    {",
    "        var vectors = LoadVectors();",
    "        var entry = (JsonObject)vectors[index]!;",
    '        var contract = (string?)entry["contract"] ?? "";',
    '        var operation = (string?)entry["operation"] ?? "";',
    '        var operationKey = $"{contract}.{operation}";',
    '        var vector = entry["vector"] as JsonObject ?? new JsonObject();',
    '        var vectorName = (string?)vector["name"] ?? "unnamed";',
    '        var vectorId = $"{operationKey}:{vectorName}";',
    '        var sync = (bool?)entry["sync"] ?? false;',
    "",
  ...
  model.vectors.forEach((entry, index) => {
    lines.push("    [Fact]");
    lines.push(
      `    public async Task ${csharpVectorSlug(index, entry)}() => await RunVector(${index});`,
    );
    lines.push("");
  });
  lines.push("}");
