...
function emitCSharpDispatchResolver(
  ...
  // Closed (no fallback) OR abstract base: an unknown discriminator is a hard
  // error, exactly as the shape switch throws. A default/open dispatch has no
  // base impl, so it yields null (harness explicit-skip) instead.
  ...
  const lines: string[] = [
    ...
    "// non-emitted file, so a forgotten slot fails to compile.",
    "// See docs: reference/vector-conformance.",
    "#nullable enable",
    "using System;",
    `using ${seamNamespace};`,
