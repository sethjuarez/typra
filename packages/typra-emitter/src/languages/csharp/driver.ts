...
export const generateCsharp = async (
  ...
  if (
    ...
    // colocated in the interface's namespace/group folder (issue #282 §8.2).
    for (const dispatched of collectDispatchedContracts(
      ...
      const resolverDir = resolverFolder
        ? `${emitTarget["test-dir"]}/${resolverFolder}`
        : emitTarget["test-dir"];
      await emitCsharpFile(
        context,
        node,
...
function collectDispatchedContracts(
  ...
  for (const entry of entries) {
    ...
    if (!decl) continue;
    if (!byContract.has(entry.contract)) {
      byContract.set(entry.contract, {
 ...
 * variant list, a consumer that forgets to attach an impl fails to COMPILE
 ...
 * (§3.1) rather than a silent registration miss. Closedness/default handling is
 * derived from the SAME `isClosedPolymorphicDispatch` / `defaultVariant` the
 * shape layer uses, so behavior and shape stay in lockstep.
 */
function emitCSharpDispatchResolver(
  ...
  // Stable, deterministic variant order (by discriminator value) for zero-diff
  // regen — the same ordering discipline the conversion-test path relies on.
  const variants = [...entry.decl.variants].sort((left, right) =>
    left.value.localeCompare(right.value),
  );
  const closed = isClosedPolymorphicDispatch(entry.decl);
  ...
  const lines: string[] = [
    ...
    "// Load switch. The provider surface below has one slot per @dispatch variant;",
    "// a consumer attaches concrete impls by satisfying it in an external,",
    "// non-emitted file, so a forgotten attachment fails to compile.",
    "// See docs: reference/vector-conformance.",
    "#nullable enable",
    "using System;",
    `using ${seamNamespace};`,
    "",
    `namespace ${seamNamespace}.Conformance;`,
    "",
    `/// <summary>`,
    `/// Consumer-attached provider of ${seam} impls, one slot per @dispatch`,
    `/// variant. Nullable: return null for a valid-but-unimplemented variant to`,
    `/// have the conformance harness skip it (never a silent registration miss).`,
    `/// </summary>`,
  ...
  lines.push(`/// behavioral twin of ${entry.contract}'s shape Load switch.`);
  lines.push(`/// </summary>`);
  ...
  if (closed || entry.decl.isAbstract) {
    lines.push(
