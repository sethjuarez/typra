...
function renderCSharpAliasSwitchArms(
  ...
  for (const [canonical, aliases] of Object.entries(parseAliases)) {
    for (const alias of aliases) {
      arms.push(`${csharpString(alias)} => ${csharpString(canonical)}, `);
    }
  }
  return arms.join("");
}
...
export function emitCSharpEnum(enumDef: EnumDef, namespace: string): string {
  ...
  lines.push("}");
  lines.push("");
  lines.push(`public static class ${csEnumName}Parser`);
  lines.push("{");
  lines.push(`    public static ${csEnumName} Parse(string value)`);
  lines.push("    {");
  ...
  lines.push("    }");
  lines.push("");
  lines.push(`    public static string ToValue(${csEnumName} value)`);
  lines.push("    {");
  lines.push("        return value switch");
  lines.push("        {");
  for (const value of enumDef.values) {
