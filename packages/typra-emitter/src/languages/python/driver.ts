...
function emitPythonVectorConformanceTest(
  ...
  const lines = [
    ...
    "    if adapter is None:",
    '        adapter = VECTOR_ADAPTERS.get(entry["operation"])',
    "    if adapter is None:",
    "        waiver = VECTOR_WAIVERS.get(operation_key) or VECTOR_WAIVERS.get(",
    '            entry["operation"]',
    "        )",
    "        if waiver:",
    ...
    '        is_sync = bool(entry.get("sync", False))',
