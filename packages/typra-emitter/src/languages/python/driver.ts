...
function emitPythonVectorConformanceTest(
  ...
  const lines = [
    ...
    "# True) must resolve synchronously -- if its adapter returns an awaitable the",
    "# vector is a hard failure. An async-capable operation (the default) stays",
    "# permissive: a plain value or an awaitable both pass.",
    ...
    '                with open(os.path.join(_BASE_DIR, raw), "r", encoding="utf-8") as handle:',
    "                    return handle.read()",
    ...
    "",
    "",
    "async def _run_vector(contract, operation, vector, is_sync):",
    ...
    "    if adapter is None:",
    '        adapter = VECTOR_ADAPTERS.get(operation)',
    "    if adapter is None:",
    "        waiver = VECTOR_WAIVERS.get(operation_key) or VECTOR_WAIVERS.get(operation)",
    "        if waiver:",
    ...
    "    per_vector_waiver = VECTOR_WAIVERS.get(vector_id) or VECTOR_WAIVERS.get(",
  ...
  model.vectors.forEach((entry, index) => {
    ...
    lines.push(
      ...
      `    await _run_vector(${JSON.stringify(entry.contract)}, ${JSON.stringify(entry.operation)}, vector, ${isSync})`,
    );
  });
  lines.push("");
  return lines.join("\n");
