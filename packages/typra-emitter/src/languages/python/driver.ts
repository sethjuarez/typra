...
function emitPythonVectorRunner(): string {
  const lines = [
    ...
    "",
    "",
    "async def run_vector(contract, operation, vector, is_sync, seam):",
...
function emitPythonVectorConformanceTest(
  ...
  model.vectors.forEach((entry, index) => {
    ...
    lines.push(
      ...
      `    await run_vector(${JSON.stringify(entry.contract)}, ${JSON.stringify(entry.operation)}, vector, ${isSync}, _SEAM)`,
    );
  });
  lines.push("");
  return lines.join("\n");
