...
function emitValidation(
  ...
  if (v.isOptional) {
    lines.push(
      `if ${varName}.${v.key} == nil || *${varName}.${v.key} != ${v.delimiter}${v.value}${v.delimiter} {`,
    );
    lines.push(
      `t.Errorf(\`Expected ${v.key} to be ${display}, got %v\`, ${varName}.${v.key})`,
    );
    lines.push(`}`);
  } else {
    lines.push(
      `if ${varName}.${v.key} != ${v.delimiter}${v.value}${v.delimiter} {`,
    );
    lines.push(
      `t.Errorf(\`Expected ${v.key} to be ${display}, got %v\`, ${varName}.${v.key})`,
    );
...
function emitExampleValidations(
  ...
  emitConcreteExampleValidations(
    ...
    sample.validations.map((validation, index) => ({
      ...validation,
      key:
        fieldNames.get(validation.sourceKey ?? validationKeys[index]) ??
        validation.key,
    })),
  );
...
function emitShorthandObjectValidation(
  ...
  const targetProp = node.properties.find(
...
function emitCollectionValidation(
  ...
  lines.push(
    `t.Fatalf("Expected ${fieldName} length to be ${values.length}, got %d", len(${expr}))`,
  );
  lines.push("}");
  ...
  if (prop.isScalar) {
    values.forEach((item, index) => {
      if (typeof item === "string") {
        ...
        lines.push(
          `t.Errorf(\`Expected ${fieldName}[${index}] to be ${goStringLiteral(item)}, got %v\`, ${expr}[${index}])`,
        );
        lines.push("}");
      }
    });
    ...
    values.forEach((item, index) => {
      if (item && typeof item === "object" && !Array.isArray(item)) {
        ...
        if (childType) {
          ...
          lines.push(
            `${localName}, ok := ${itemExpr}.(${pkg}.${childType.typeName.name})`,
          );
          lines.push("if !ok {");
          lines.push(
            `t.Fatalf("Expected ${fieldName}[${index}] to be ${pkg}.${childType.typeName.name}, got %T", ${itemExpr})`,
          );
          lines.push("}");
          emitStructuredValidations(
            lines,
        ...
        emitReflectiveObjectValidation(
          lines,
          itemExpr,
          `${fieldName}[${index}]`,
          item,
...
function emitReflectiveObjectValidation(
  ...
  for (const [key, expected] of Object.entries(sample)) {
    if (typeof expected === "string") {
      lines.push(
        `${reflectiveHelper}(t, ${expr}, ${goStringLiteral(goFieldName(key))}, ${goStringLiteral(expected)}, ${goStringLiteral(`${displayName}.${goFieldName(key)}`)})`,
      );
    }
  }
}
...
function emitScalarSampleValidation(
  ...
  if (typeof expected === "string") {
    if (isOptional) {
      lines.push(
        `if ${expr} == nil || *${expr} != ${goStringLiteral(expected)} {`,
      );
      lines.push(
        `t.Errorf(\`Expected ${fieldName} to be ${goStringLiteral(expected)}, got %v\`, ${expr})`,
      );
      lines.push("}");
    } else {
...
function emitDictValidation(
  ...
  for (const [key, expected] of Object.entries(value)) {
    if (typeof expected === "string") {
      lines.push(
        `if got := ${expr}[${goStringLiteral(key)}]; got != ${goStringLiteral(expected)} {`,
      );
      lines.push(
        `t.Errorf(\`Expected ${fieldName}[${goStringLiteral(key)}] to be ${goStringLiteral(expected)}, got %v\`, got)`,
      );
      lines.push("}");
    }
  }
...
function emitPolymorphicValidation(
  ...
  lines.push("if !ok {");
  lines.push(
    `t.Fatalf("Expected ${fieldName} to be ${pkg}.${childType.typeName.name}, got %T", ${expr})`,
  );
  lines.push("}");
  emitStructuredValidations(
    lines,
...
function emitNestedObjectValidation(
  ...
  for (const prop of node.properties) {
    ...
    if (typeof expected === "string") {
      ...
      if (prop.isOptional) {
        lines.push(
          `if ${nestedExpr} == nil || *${nestedExpr} != ${goStringLiteral(expected)} {`,
        );
        lines.push(
          `t.Errorf(\`Expected ${fieldName}.${nestedField} to be ${goStringLiteral(expected)}, got %v\`, ${nestedExpr})`,
        );
        lines.push("}");
      } else {
...
function emitWireValidationTest(
  ...
  for (const provider of providers) {
    lines.push("");
    lines.push(
      `${provider}Wire := instance.ToWire(${goStringLiteral(provider)})`,
    );
    for (const mapping of mappings.filter(
      (entry) => entry.provider === provider,
    )) {
      lines.push(
        `if _, ok := ${provider}Wire[${goStringLiteral(mapping.wireName)}]; !ok {`,
      );
      lines.push(
        `t.Errorf("Expected ${provider} wire output to include ${mapping.wireName}")`,
      );
      lines.push("}");
      if (mapping.fieldName !== mapping.wireName) {
        lines.push(
          `if _, ok := ${provider}Wire[${goStringLiteral(mapping.fieldName)}]; ok {`,
        );
        lines.push(
          `t.Errorf("Expected ${provider} wire output to omit source field ${mapping.fieldName}")`,
        );
        lines.push("}");
      }
    }

    // Round-trip: FromWire must invert ToWire so re-serializing the reloaded
    // instance reproduces the same provider-native payload.
    lines.push(
      `${provider}Restored, ${provider}Err := ${pkg}.${typeName}FromWire(${goStringLiteral(provider)}, ${provider}Wire, ctx)`,
    );
    lines.push(`if ${provider}Err != nil {`);
    lines.push(
      `t.Fatalf("Expected ${provider} FromWire to succeed, got %v", ${provider}Err)`,
    );
    lines.push("}");
    lines.push(
      `if ${provider}RoundTrip := ${provider}Restored.ToWire(${goStringLiteral(provider)}); !reflect.DeepEqual(${provider}RoundTrip, ${provider}Wire) {`,
    );
    lines.push(
      `t.Errorf("Expected ${provider} FromWire round-trip to preserve the wire payload, got %v want %v", ${provider}RoundTrip, ${provider}Wire)`,
    );
    lines.push("}");
  }
...
function needsReflectiveCollectionValidation(
  node: TypeNode,
...
function emitReflectiveFieldHelpers(lines: string[], helperName: string): void {
  lines.push("");
  lines.push(
    `func ${helperName}(t *testing.T, value interface{}, fieldName string, expected string, displayName string) {`,
  );
  lines.push("t.Helper()");
...
export function emitGoTest(
  ...
  const needsReflect =
    needsReflectiveCollectionValidation(ctx.node, ctx.examples) ||
    (!isAbstract &&
      ctx.examples.length > 0 &&
      wireTestMappings(ctx.node, ctx.examples[0]).length > 0);
...
function emitJsonUnmarshal(lines: string[], varName: string = "data"): void {
  ...
  lines.push(
    `if err := json.Unmarshal([]byte(jsonData), &${varName}); err != nil {`,
  );
  lines.push(`t.Fatalf("Failed to parse JSON: %v", err)`);
  lines.push(`}`);
}

function emitInvalidFromJSONTest(
  lines: string[],
  typeName: string,
  pkg: string,
): void {
  lines.push(
    `// Test${typeName}FromJSONInvalid rejects malformed JSON instead of silently defaulting`,
  );
...
function emitYamlUnmarshal(lines: string[], varName: string = "data"): void {
  lines.push(`var ${varName} map[string]interface{}`);
  lines.push(
    `if err := yaml.Unmarshal([]byte(yamlData), &${varName}); err != nil {`,
  );
...
function emitLoadCall(
  ...
  lines.push(
    `${instanceVar}, err := ${pkg}.Load${typeName}(${dataVar}, ${ctxVar})`,
  );
...
function emitLoadJSONTest(
  ...
  lines.push(
    `// Test${typeName}LoadJSON${suffix} tests loading ${typeName} from JSON`,
  );
...
function emitLoadYAMLTest(
  ...
  lines.push(
    `// Test${typeName}LoadYAML${suffix} tests loading ${typeName} from YAML`,
  );
...
function emitFromJSONTest(
  ...
  lines.push(
    `// Test${typeName}FromJSON${suffix} tests loading ${typeName} through the generated JSON helper`,
  );
  ...
  lines.push(`if err != nil {`);
  lines.push(
    `t.Fatalf("Failed to load ${typeName} from JSON helper: %v", err)`,
  );
  lines.push(`}`);
  if (isAbstract) {
    emitAbstractExampleValidations(lines, sample.validations.length > 0);
...
function emitFromYAMLTest(
  ...
  lines.push(
    `// Test${typeName}FromYAML${suffix} tests loading ${typeName} through the generated YAML helper`,
  );
  ...
  lines.push(`if err != nil {`);
  lines.push(
    `t.Fatalf("Failed to load ${typeName} from YAML helper: %v", err)`,
  );
  lines.push(`}`);
  if (isAbstract) {
    emitAbstractExampleValidations(lines, sample.validations.length > 0);
...
function emitRoundtripTest(
  ...
  lines.push(
    `// Test${typeName}Roundtrip${suffix} tests load -> save -> load produces equivalent data`,
  );
...
function emitToJSONTest(
  ...
  lines.push(
    `// Test${typeName}ToJSON${suffix} tests that ToJSON produces valid JSON`,
  );
...
function emitToYAMLTest(
  ...
  lines.push(
    `// Test${typeName}ToYAML${suffix} tests that ToYAML produces valid YAML`,
  );
...
function emitCoercionTest(
  ...
  lines.push(
    `// Test${typeName}From${title}${suffix} tests loading ${typeName} from ${alt.scalarType}`,
  );
  ...
  lines.push(`if err != nil {`);
  lines.push(
    `t.Fatalf("Failed to load ${typeName} from ${alt.scalarType}: %v", err)`,
  );
  lines.push(`}`);
  if (isAbstract) {
    lines.push(
  ...
  lines.push("if err != nil {");
  lines.push(
    `t.Fatalf("Failed to load ${typeName} from ${alt.scalarType} JSON helper: %v", err)`,
  );
  lines.push("}");
  if (isAbstract) {
    lines.push(
  ...
  lines.push("if err != nil {");
  lines.push(
    `t.Fatalf("Failed to load ${typeName} from ${alt.scalarType} YAML helper: %v", err)`,
  );
  lines.push("}");
  if (isAbstract) {
    lines.push(
