...
export function emitGoFileContent(
  ...
  // Serialization imports (encoding/json, yaml) and the fmt/math they pull in
  // are only needed when at least one type in the file actually emits load/save.
  // A file of shape-only (non-`@serializable`) types must not request them, or
  // Go rejects the file for unused imports.
  const anySerialized = types.some((t) => t.serialized);
  const needsContext = types.some((type) =>
    type.methods.some((method) => method.runtimeCancellable),
  );
  const needsNamedCollections = types.some(
    (type) =>
      type.serialized &&
      type.collectionHelpers.some((helper) => helper.hasNameProperty),
  );
  const needsRequiredComplexValidation = types.some(
    (type) =>
      type.serialized &&
  ...
  const needsFmt =
    ...
    types.some((type) => type.serialized && type.polymorphicDispatch) ||
...
function emitHeader(
  ...
  needsSerialization: boolean = true,
): void {
  ...
  if (needsSerialization) {
    thirdParty.push('\t"gopkg.in/yaml.v3"');
  }
...
function emitTypeBlock(
  ...
  // Serialization surface (load/save + format wrappers) is opt-in: emitted only
