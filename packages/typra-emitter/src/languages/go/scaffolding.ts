...
export function emitGoContext(ctx: GoContextContext): string {
  ...
  lines.push("}");
  lines.push("");
  lines.push("// At creates a child context for a nested schema field.");
  lines.push("func (ctx *LoadContext) At(segment string) *LoadContext {");
  lines.push('\tif ctx == nil || ctx.Path == "" {');
  lines.push("\t\treturn &LoadContext{Path: segment}");
  lines.push("\t}");
  ...
  lines.push("\tCollectionFormat string");
  lines.push("\tUseShorthand bool");
  lines.push("}");
  lines.push("");
