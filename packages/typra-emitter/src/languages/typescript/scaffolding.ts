 ...
 */
export function emitTypeScriptIndex(baseTypes: TypeNode[], _types: TypeNode[], includeWireTypes = false): string {
  const lines: string[] = [];
    ...
    if (!group) {
      ...
      for (const type of groupTypes) {
        ...
        if (type.childTypes.length > 0) {
          ...
          const typeExports = includeWireTypes ? exports.map(name => `${name}Wire`) : [];
          lines.push("");
          ...
          if (typeExports.length > 0) {
            lines.push(`export type { ${typeExports.join(", ")} } from "./${toKebabCase(type.typeName.name)}";`);
          }
        } else {
          lines.push(`${exportKeyword} { ${type.typeName.name} } from "./${toKebabCase(type.typeName.name)}";`);
          if (includeWireTypes && !type.isProtocol) {
            lines.push(`export type { ${type.typeName.name}Wire } from "./${toKebabCase(type.typeName.name)}";`);
          }
        }
      }
    } else {
      ...
      for (const type of groupTypes) {
        ...
        if (allNames.length > 1) {
          ...
          if (includeWireTypes && !type.isProtocol) {
            lines.push(`export type { ${allNames.map(name => `${name}Wire`).join(", ")} } from "./${group}/${toKebabCase(type.typeName.name)}";`);
          }
        } else {
          lines.push(`${exportKeyword} { ${type.typeName.name} } from "./${group}/${toKebabCase(type.typeName.name)}";`);
          if (includeWireTypes && !type.isProtocol) {
            lines.push(`export type { ${type.typeName.name}Wire } from "./${group}/${toKebabCase(type.typeName.name)}";`);
          }
        }
      }
    }
 ...
 */
export function emitTypeScriptGroupIndex(group: string, groupNodes: TypeNode[], includeWireTypes = false): string {
  const lines: string[] = [];
  lines.push("// Copyright (c) Microsoft. All rights reserved.");
  lines.push("// WARNING: This is an auto-generated file. DO NOT EDIT THIS FILE DIRECTLY.");
  lines.push("");
  ...
  for (const type of groupNodes) {
    ...
    if (allNames.length > 1) {
      ...
      lines.push(`} from "./${toKebabCase(type.typeName.name)}";`);
      if (includeWireTypes && !type.isProtocol) {
        lines.push(`export type { ${allNames.map(name => `${name}Wire`).join(", ")} } from "./${toKebabCase(type.typeName.name)}";`);
      }
    } else {
      lines.push(`${exportKeyword} { ${type.typeName.name} } from "./${toKebabCase(type.typeName.name)}";`);
      if (includeWireTypes && !type.isProtocol) {
