...
function emitEnumParser(
  ...
  lines.push(
    `function ${enumParserName(enumDef.name)}(value: unknown): ${enumDef.name} {`,
  );
  ...
  if (enumDef.isOpen) {
    ...
    lines.push(
      `      throw new Error(\`Invalid ${enumDef.name} value: \${text}\`);`,
    );
  }
  lines.push("  }");
  lines.push("}");
...
function emitUnknownCarrier(type: TypeDecl, lines: string[]): void {
  ...
  lines.push(` *`);
  lines.push(
    ` * The unrecognized value stays on \`${type.polymorphicDispatch!.discriminatorField}\` and every`,
  );
  lines.push(
    ` * key the schema does not declare is preserved verbatim, so an unknown ${parentName}`,
  );
  ...
  lines.push(
    `    instance.raw = ${parentName}.cloneRawValue(data) as Record<string, unknown>;`,
  );
...
function emitMethodHelpersInterface(type: TypeDecl, lines: string[]): void {
  ...
  for (const m of type.methods) {
    ...
    const params = Object.entries(m.params).map(
      ([pName, pType]) => `${pName}: ${returnType(pType)}`,
    );
...
function emitZodSchemas(
  ...
  const name = type.typeName.name;
  lines.push(
    `  static readonly wireObjectSchema: z.ZodObject<any> = ${zodObjectExpression(type, allTypes)};`,
  );
  lines.push(
    `  static readonly wireObjectSchemaWithoutName: z.ZodObject<any> = ${zodObjectExpression(type, allTypes, "name")};`,
  );
  lines.push(
    `  static readonly wireSchema: z.ZodType<Record<string, unknown>> = z.lazy(() => ${zodWireSchemaExpression(type)});`,
  );
  lines.push(`  static readonly schema = z.any().transform((data, ctx) => {`);
  lines.push(`    try {`);
  lines.push(
    `      return ${name}.load(data as Record<string, unknown>).save();`,
  );
...
function zodObjectExpression(
  ...
  const entries = type.save.assignments
    ...
    .map(
      (a) =>
        `    "${a.targetName}": ${zodFieldSchema(type, a, allTypes)}${a.isOptional ? ".optional()" : ""},`,
    );
  const objectBody =
    entries.length > 0 ? `{\n${entries.join("\n")}\n  }` : "{}";
  const object = type.base
    ...
    : `z.object(${objectBody})`;
  return absorbsUnknownValues(type.polymorphicDispatch)
    ? `${object}.passthrough()`
    : object;
}
...
function zodFieldSchema(
  ...
  const baseType = type.base
    ? allTypes.find((candidate) => candidate.typeName.name === type.base!.name)
    : undefined;
  const discriminatorField =
    type.polymorphicDispatch?.discriminatorField ??
    baseType?.polymorphicDispatch?.discriminatorField;
  if (
    field &&
    type.base !== null &&
  ...
  if (field?.enumName && field.allowedValues.length > 0) {
    return field.isOpenEnum
      ? "z.string()"
      : zodEnumExpression(field.allowedValues);
  }
  ...
  switch (cat.kind) {
    ...
    case "collection_complex": {
      const helper = type.collectionHelpers.find(
        (candidate) => candidate.propertyName === assignment.fieldName,
      );
...
function emitProtocolInterface(type: TypeDecl, lines: string[]): void {
  ...
  for (const method of type.methods) {
    ...
    if (method.sync) {
      ...
      lines.push(
        `  ${method.name}${optMark}(${params.join(", ")}): Promise<${ret}>;`,
      );
    }
  }
...
function emitConstructor(type: TypeDecl, lines: string[]): void {
  ...
  for (const field of type.fields) {
    ...
    if (
      ...
      cat.kind === "collection_scalar" ||
      cat.kind === "collection_complex"
    ) {
      lines.push(`    this.${field.name} = init?.${field.name} ?? [];`);
      ...
      const dv =
        ...
        field.allowedValues.includes(field.defaultValue)
          ? field.defaultValue
...
function emitLoadMethod(type: TypeDecl, lines: string[]): void {
  ...
  lines.push(
    `  static load(data: Record<string, unknown>, context?: LoadContext): ${name} {`,
  );
  ...
  for (const a of type.load.assignments) {
    ...
    lines.push(
      `    if (data["${a.sourceName}"] === undefined || data["${a.sourceName}"] === null) {`,
    );
    lines.push(
      `      throw new Error(\`\${context.at("${a.sourceName}").path}: missing required field\`);`,
    );
    lines.push("    }");
  }
  ...
  if (type.polymorphicDispatch?.defaultVariant?.isSelfReference) {
    ...
    lines.push(
      `      instance.raw = ${name}.cloneRawValue(data) as Record<string, unknown>;`,
    );
    lines.push("    }");
  }
...
function emitCoercionBody(
  ...
  if (c.needsDispatch && type.polymorphicDispatch) {
    ...
    lines.push(
      `${indent}return ${typeName}.loadKind({ ${dictFields.join(", ")} } as Record<string, unknown>, context);`,
    );
  } else {
    ...
    for (const a of c.assignments) {
      if (a.isInput) {
        ...
        const castType =
          targetField?.enumName && targetField.allowedValues.length > 0
    ...
    lines.push(
      `${indent}  return context.processOutput(instance) as ${typeName};`,
    );
    lines.push(`${indent}}`);
...
function emitCollectionLoadHelper(
  ...
  lines.push(
    `  static ${methodName}(data: Record<string, unknown>[] | unknown[], context?: LoadContext): ${elemName}[] {`,
  );
  lines.push(
    `    context ??= new LoadContext({ path: "${helper.propertyName}" });`,
  );
  ...
  lines.push(
    `          result.push(${elemName}.load({ name: k, ...(v as Record<string, unknown>) }, context.at(k)));`,
  );
  lines.push("        } else {");
  ...
  lines.push("    }");
  lines.push(
    `    return data.map((item, index) => ${elemName}.load(item as Record<string, unknown>, context.atIndex(index)));`,
  );
  lines.push("  }");
  lines.push("");
}
...
function emitTsEntryShorthandArms(
  ...
  const load = (expr: string) =>
    lines.push(
      `          result.push(${elemName}.load(${expr}, context.at(k)));`,
    );
...
function emitCollectionSaveHelper(
  ...
  lines.push(
    `  static ${loadMethodName}(items: ${elemName}[], context?: SaveContext): Record<string, unknown>[] | Record<string, unknown> {`,
  );
  lines.push("    if (!context) {");
  ...
  if (helper.hasNameProperty) {
    ...
    lines.push(
      `      const shorthand = (item.constructor as typeof ${elemName}).shorthandProperty;`,
    );
    lines.push(
...
function emitSaveMethod(type: TypeDecl, lines: string[]): void {
  ...
  if (type.save.hasBase) {
    ...
    lines.push(
      `    const result = ${name}.cloneRawValue(obj.raw) as Record<string, unknown>;`,
    );
  } else {
    lines.push("    const result: Record<string, unknown> = {};");
  }
  ...
  for (const a of type.save.assignments) {
    if (shouldOmitAbsentOnSave(a, "always-check")) {
      lines.push(
        `    if (obj.${a.fieldName} !== undefined && obj.${a.fieldName} !== null) {`,
      );
...
function emitFromJson(name: string, type: TypeDecl, lines: string[]): void {
  lines.push(
    `  static fromJson(json: string, context?: LoadContext): ${name} {`,
  );
  ...
  if (type.load.coercions.length > 0) {
    ...
    lines.push(
      `    return ${name}.load(data as Record<string, unknown>, context);`,
    );
  } else {
    lines.push(
      `    return ${name}.load(data as Record<string, unknown>, context);`,
