...
/**
 * Append trailing commas to a `JSON.stringify(value, null, 2)` rendering so the embedded literal
 * matches prettier's `trailingComma: "all"` layout (keeping native `format:false` output idempotent).
 * A line receives a trailing comma when the next non-blank line begins a collection closer (`}`/`]`)
 * and the line is not itself an opener/already-terminated — always safe for pure JSON data.
 */
function jsonWithTrailingCommas(text: string): string {
  const lines = text.split("\n");
  return lines
    .map((line, i) => {
      let j = i + 1;
      while (j < lines.length && lines[j].trim() === "") j++;
      if (j >= lines.length) return line;
      const nextFirst = lines[j].trim()[0];
      if (nextFirst !== "}" && nextFirst !== "]") return line;
      const last = line.trimEnd().slice(-1);
      if (last === "" || last === "," || last === "{" || last === "[") return line;
      return `${line},`;
    })
    .join("\n");
}
