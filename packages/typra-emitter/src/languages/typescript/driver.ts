...
export const generateTypeScript = async (
  ...
  if (emitTarget["test-dir"]) {
    ...
    if ((options?.callableVectors?.vectors.length ?? 0) > 0) {
      ...
      // every scalar-eligible plain seam, emit `run<Seam>Conformance(seam)` that
      ...
      // beside the stringly runner. Scalar-only for the first slice (see
      // `isScalarSeamEntry`), keeping it zero-diff on real surfaces whose eligible
      // plain seams take model shapes.
 ...
 *
 * For every scalar-eligible plain seam it emits ONE exported async function
 ...
 * helper until a typed test calls it. Scalar-only for the first slice (shared
 * `isScalarSeamEntry`), keeping it zero-diff on real surfaces whose eligible
 * plain seams take model shapes.
 */
function emitTypeScriptVectorConformanceEntrypoint(
  ...
  const lines: string[] = [
    ...
    `import type { ${seamNames.join(", ")} } from "./index";`,
    "",
  ];
