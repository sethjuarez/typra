...
export const generateRust = async (
  ...
  for (const group of sourceGroupPaths) {
    const groupModContent = emitRustGroupMod(
      groupModuleNames.get(group) ?? [],
      immediateChildModules(group, sourceGroupPaths),
    );
    await emitRustFile(
      context,
      "mod.rs",
...
/**
 * Emit the Rust closed-loop `@vector` behavioral conformance suite. Each vector
 * is replayed through a runtime-authored adapter module included via `#[path]`
 * from the target's `vector-adapter-path` option (a source file relative to the
 * generated test). A vector with no adapter and no explicit waiver is a hard
 * failure — this suite never skips silently.
 */
function emitRustVectorConformanceTest(
  ...
  const lines: string[] = [
    ...
    "// must be registered `Invoke::Sync` — registering it `Invoke::Async` is a hard",
    "// failure. An async-capable operation (the default) accepts either variant.",
    ...
    "",
    `const VECTOR_CONFORMANCE_PAYLOAD: &str = ${payloadLiteral};`,
    "",
    "fn vc_load() -> Vec<Value> {",
    "    serde_json::from_str(VECTOR_CONFORMANCE_PAYLOAD)",
    '        .expect("failed to decode embedded vectors")',
    "}",
    "",
    ...
    '    let vector = entry.get("vector").cloned().unwrap_or(Value::Null);',
    '    let vector_name = vector.get("name").and_then(|v| v.as_str()).unwrap_or("unnamed");',
    '    let vector_id = format!("{}:{}", operation_key, vector_name);',
    '    let sync = entry.get("sync").and_then(|v| v.as_bool()).unwrap_or(false);',
    "",
    "    let adapters = vector_adapters::adapters();",
    ...
    "        None => {",
    "            let waivers = vector_adapters::waivers();",
    "            let waiver = waivers",
    "                .get(operation_key.as_str())",
    ...
    '                assert_eq!(normalize(observed), expected, "{} error mismatch", vector_id);',
    "            }",
    "        }",
    "    } else {",
    "        match vc_invoke(adapter, &input, &ctx).await {",
