...
function emitRustVectorConformanceTest(
  ...
  const lines: string[] = [
    ...
    "}",
    "",
    "async fn vc_run_vector(contract: &str, operation: &str, vector: Value, sync: bool) {",
  ...
  model.vectors.forEach((entry, index) => {
    ...
    lines.push(
      `    vc_run_vector(${JSON.stringify(entry.contract)}, ${JSON.stringify(
        entry.operation,
      )}, vector, ${entry.sync ? "true" : "false"}).await;`,
    );
    lines.push("}");
    lines.push("");
  });
