...
description: Contract vocabulary and adoption rules for Typra's principle-led roadmap.
draft: false
---
...
- `input`, compared structurally as opaque evidence (not normalized through target load/save),
...
The oracle succeeds when the observed transcript matches the expected transcript derived from the vector. Failures must include the vector id, target runtime, expected transcript, observed transcript, and the mismatched path so drift is actionable across languages. Generated TypeScript and Python vector conformance tests obtain a runtime-supplied **adapter** for each operation through a required registration hook, invoke it with the vector `input`, and canonically compare the actual result against `expected`. A missing adapter is a build or hard failure — never a silent skip — unless the operation carries a tracked waiver. Because vector `input`/`expected` are opaque conformance evidence (not typed against the operation's parameters), they are compared structurally and are **not** round-tripped through model-typed `load().save()` surfaces. Doing so would force vector authors to pre-normalize inputs into canonical `save()` form — coupling opaque evidence to a target's default-serialization rules. Model-typed data fidelity is proven by `@sample` data conformance instead. Every `@vector`-bound operation is as enforced as a type round-trip test; see [Vector conformance adapters](/reference/vector-conformance/) for the adapter registration contract, collaborator injection seams, normalization convention, and waiver ramp.
...
HTTP transport projection (`fastapi`, `starlette`, `httpx`, `fetch`) is experimental. The IR and generated route/consumer surfaces may still change. It builds on the stable callable/vector foundation but is not itself production-ready.
:::
