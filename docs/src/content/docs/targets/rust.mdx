...
| `native-serialization` | `"serde"` emits feature-gated `Serialize`/`Deserialize` impls; `"none"` opts out. Omission preserves the legacy cfg-gated serde surface for compatibility. |
