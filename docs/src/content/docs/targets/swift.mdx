...
`@@method` signatures emit throwing stubs that must be completed by hand-authored extension code. Until implemented, they throw `TypraRuntimeError.unsupported`, so generated code compiles while keeping domain behavior in the consuming app.
