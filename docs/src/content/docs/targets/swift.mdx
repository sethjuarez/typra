...
`native-serialization: "codable"` interop is experimental. It is not yet idempotency-locked under the output hygiene guard, and its generated shape may still change. The production-ready Swift output is the core `TypraModel` surface — `load`/`save` and the `fromJSON`/`toJSON`/`fromYAML`/`toYAML` helpers.
:::
...
Swift generation is part of the fixture suite. Fixture validation runs `swift build` and `swift test` against the generated package, and a Swift toolchain is required just like the other targets' toolchains — a missing `swift` is a hard failure, not a skip.
