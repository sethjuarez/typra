...
`native-serialization: "jackson"` interop is experimental. It is not yet idempotency-locked under the output hygiene guard, and its generated shape may still change. The production-ready Java output is the core model surface — `load`/`save` and the `fromJson`/`toJson`/`fromYaml`/`toYaml` helpers.
:::
