...
`native-serialization: "pydantic"` is opt-in and requires Pydantic v2 in the consuming Python environment. Typra still treats generated `load()` and `save()` as the authoritative contract surface: the Pydantic `model_validate()`, `model_dump()`, and `model_dump_json()` helpers delegate to those methods rather than introducing a second loader or serializer with different semantics.
