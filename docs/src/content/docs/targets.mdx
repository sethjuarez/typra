...
| `Rust` | serde-oriented structs with `from_json`/`to_json`, `from_yaml`/`to_yaml`, provider wire mapping, and optional case-insensitive enum parsing. |
| `Swift` | A SwiftPM package with `TypraModel` types, `fromJSON`/`toJSON`, `fromYAML`/`toYAML`, scalar coercion, polymorphic enums, provider wire mapping, and XCTest tests. |
...
| `Swift` | Emits a SwiftPM package with a Yams dependency for YAML; requires a Swift toolchain to build and test. |
